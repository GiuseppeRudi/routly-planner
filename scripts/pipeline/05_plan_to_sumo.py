...
def open_event_map_before_sumo(config, features: FeatureConfig, plan_kinds: list[str], mapping: dict[str, Any], station_ids: list[str]) -> None:
    ...
    print("\nOpening the interactive event comparison map before SUMO launch...")

    # Load the base and event-adjusted plans generated by the planning step.
    ...
    # Load road events recorded by the LLM planning step.
    ...
    # Recover start and goal from the experiment scenario.
...
def main() -> None:
    ...
    for plan_kind, plan_path in plan_runs:
        ...
        if plan_kind == "dynamic" and features.llm_events.enabled:
            ...
            if log_path.exists():
                try:
                    ...
                    print(
                        "Dynamic SUMO network: applied "
                        f"{len(blocked_roads_set)} road closures and "
                        f"{len(slowed_roads_map)} speed reductions."
                    )
                except Exception as e:
                    print(f"WARNING: could not apply LLM road events ({e})")
        ...
        if edg_path.exists():
            try:
                ...
                for edge in root.findall("edge"):
                    r_id = edge.get("id")
                    if r_id in blocked_roads_set:
                        edge.set("color", "230,76,60")
                    elif r_id in slowed_roads_map:
                        edge.set("color", "241,196,15")
                    else:
                        edge.set("color", "52,152,219")
        ...
        # Compile the SUMO network after applying event colors and restrictions.
