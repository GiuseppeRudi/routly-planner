;; ============================================================
;;  PROBLEM: exp_300_nodes_05_planner_comp_line_comp_20260715_191322
;;  Features: tl_cong-pddl-static_llm_macro
;;  Start: loc_0189
;;  Goal:  loc_0170
;; ============================================================

(define (problem exp_300_nodes_05_planner_comp_line_comp_20260715_191322_dynamic)
  (:domain road-network)

  (:objects
    car1 - vehicle
    loc_0000 - location
    loc_0001 - location
    loc_0002 - location
    loc_0003 - location
    loc_0004 - location
    loc_0005 - location
    loc_0006 - location
    loc_0007 - location
    loc_0008 - location
    loc_0009 - location
    loc_0010 - location
    loc_0011 - location
    loc_0012 - location
    loc_0013 - location
    loc_0014 - location
    loc_0015 - location
    loc_0016 - location
    loc_0017 - location
    loc_0018 - location
    loc_0019 - location
    loc_0020 - location
    loc_0021 - location
    loc_0022 - location
    loc_0023 - location
    loc_0024 - location
    loc_0025 - location
    loc_0026 - location
    loc_0027 - location
    loc_0028 - location
    loc_0029 - location
    loc_0030 - location
    loc_0031 - location
    loc_0032 - location
    loc_0033 - location
    loc_0034 - location
    loc_0035 - location
    loc_0037 - location
    loc_0038 - location
    loc_0039 - location
    loc_0040 - location
    loc_0041 - location
    loc_0042 - location
    loc_0043 - location
    loc_0044 - location
    loc_0045 - location
    loc_0046 - location
    loc_0047 - location
    loc_0048 - location
    loc_0049 - location
    loc_0050 - location
    loc_0051 - location
    loc_0052 - location
    loc_0053 - location
    loc_0054 - location
    loc_0055 - location
    loc_0056 - location
    loc_0057 - location
    loc_0058 - location
    loc_0059 - location
    loc_0060 - location
    loc_0061 - location
    loc_0062 - location
    loc_0063 - location
    loc_0064 - location
    loc_0065 - location
    loc_0066 - location
    loc_0067 - location
    loc_0068 - location
    loc_0069 - location
    loc_0070 - location
    loc_0071 - location
    loc_0073 - location
    loc_0074 - location
    loc_0075 - location
    loc_0076 - location
    loc_0077 - location
    loc_0078 - location
    loc_0079 - location
    loc_0080 - location
    loc_0081 - location
    loc_0082 - location
    loc_0083 - location
    loc_0084 - location
    loc_0085 - location
    loc_0086 - location
    loc_0087 - location
    loc_0088 - location
    loc_0089 - location
    loc_0090 - location
    loc_0091 - location
    loc_0092 - location
    loc_0093 - location
    loc_0094 - location
    loc_0095 - location
    loc_0096 - location
    loc_0097 - location
    loc_0098 - location
    loc_0099 - location
    loc_0100 - location
    loc_0101 - location
    loc_0102 - location
    loc_0103 - location
    loc_0104 - location
    loc_0105 - location
    loc_0106 - location
    loc_0107 - location
    loc_0108 - location
    loc_0109 - location
    loc_0110 - location
    loc_0111 - location
    loc_0112 - location
    loc_0113 - location
    loc_0114 - location
    loc_0115 - location
    loc_0116 - location
    loc_0117 - location
    loc_0118 - location
    loc_0119 - location
    loc_0120 - location
    loc_0121 - location
    loc_0122 - location
    loc_0123 - location
    loc_0124 - location
    loc_0125 - location
    loc_0126 - location
    loc_0127 - location
    loc_0128 - location
    loc_0129 - location
    loc_0130 - location
    loc_0131 - location
    loc_0132 - location
    loc_0133 - location
    loc_0134 - location
    loc_0135 - location
    loc_0136 - location
    loc_0137 - location
    loc_0138 - location
    loc_0139 - location
    loc_0140 - location
    loc_0141 - location
    loc_0142 - location
    loc_0143 - location
    loc_0144 - location
    loc_0145 - location
    loc_0146 - location
    loc_0147 - location
    loc_0148 - location
    loc_0149 - location
    loc_0152 - location
    loc_0153 - location
    loc_0154 - location
    loc_0155 - location
    loc_0156 - location
    loc_0157 - location
    loc_0158 - location
    loc_0159 - location
    loc_0160 - location
    loc_0161 - location
    loc_0162 - location
    loc_0163 - location
    loc_0164 - location
    loc_0165 - location
    loc_0166 - location
    loc_0167 - location
    loc_0168 - location
    loc_0169 - location
    loc_0170 - location
    loc_0171 - location
    loc_0172 - location
    loc_0173 - location
    loc_0174 - location
    loc_0175 - location
    loc_0176 - location
    loc_0177 - location
    loc_0178 - location
    loc_0179 - location
    loc_0180 - location
    loc_0181 - location
    loc_0182 - location
    loc_0183 - location
    loc_0184 - location
    loc_0185 - location
    loc_0187 - location
    loc_0188 - location
    loc_0189 - location
    loc_0191 - location
    loc_0192 - location
    loc_0193 - location
    loc_0194 - location
    loc_0195 - location
    loc_0196 - location
    loc_0197 - location
    loc_0198 - location
    loc_0199 - location
    loc_0200 - location
    loc_0201 - location
    loc_0202 - location
    loc_0203 - location
    loc_0204 - location
    loc_0205 - location
    loc_0206 - location
    loc_0207 - location
    loc_0208 - location
    loc_0209 - location
    loc_0210 - location
    loc_0211 - location
    loc_0212 - location
    loc_0213 - location
    loc_0214 - location
    loc_0215 - location
    loc_0216 - location
    loc_0217 - location
    loc_0218 - location
    loc_0219 - location
    loc_0220 - location
    loc_0221 - location
    loc_0222 - location
    loc_0223 - location
    loc_0224 - location
    loc_0226 - location
    loc_0227 - location
    loc_0228 - location
    loc_0229 - location
    loc_0230 - location
    loc_0231 - location
    loc_0232 - location
    loc_0233 - location
    loc_0234 - location
    loc_0235 - location
    loc_0236 - location
    loc_0237 - location
    loc_0238 - location
    loc_0239 - location
    loc_0240 - location
    loc_0241 - location
    loc_0242 - location
    loc_0243 - location
    loc_0244 - location
    loc_0245 - location
    loc_0247 - location
    loc_0248 - location
    loc_0249 - location
    loc_0250 - location
    loc_0251 - location
    loc_0253 - location
    loc_0254 - location
    loc_0255 - location
    loc_0257 - location
    loc_0258 - location
    loc_0259 - location
    loc_0260 - location
    loc_0261 - location
    loc_0262 - location
    loc_0263 - location
    loc_0264 - location
    loc_0265 - location
    loc_0266 - location
    loc_0267 - location
    loc_0268 - location
    loc_0269 - location
    loc_0270 - location
    loc_0271 - location
    loc_0272 - location
    loc_0273 - location
    loc_0274 - location
    loc_0275 - location
    loc_0276 - location
    loc_0277 - location
    loc_0278 - location
    loc_0279 - location
    loc_0280 - location
    loc_0281 - location
    loc_0282 - location
    loc_0283 - location
    loc_0285 - location
    loc_0286 - location
    loc_0287 - location
    loc_0288 - location
    loc_0289 - location
    loc_0290 - location
    loc_0291 - location
    loc_0292 - location
    loc_0293 - location
    loc_0294 - location
    loc_0295 - location
    loc_0296 - location
    loc_0297 - location
    loc_0298 - location
    loc_0299 - location
    macro_0000 - road_type_macro_0000
    macro_0001 - road_type_macro_0001
    macro_0002 - road_type_macro_0002
    macro_0003 - road_type_macro_0003
    macro_0004 - road_type_macro_0004
    macro_0005 - road_type_macro_0005
    macro_0006 - road_type_macro_0006
    macro_0007 - road_type_macro_0007
    macro_0008 - road_type_macro_0008
    macro_0009 - road_type_macro_0009
    macro_0010 - road_type_macro_0010
    macro_0011 - road_type_macro_0011
    macro_0012 - road_type_macro_0012
    macro_0013 - road_type_macro_0013
    macro_0014 - road_type_macro_0014
    macro_0015 - road_type_macro_0015
    macro_0016 - road_type_macro_0016
    macro_0017 - road_type_macro_0017
    macro_0018 - road_type_macro_0018
    macro_0019 - road_type_macro_0019
    macro_0020 - road_type_macro_0020
    macro_0021 - road_type_macro_0021
    macro_0022 - road_type_macro_0022
    macro_0023 - road_type_macro_0023
    macro_0024 - road_type_macro_0024
    macro_0025 - road_type_macro_0025
    macro_0026 - road_type_macro_0026
    macro_0027 - road_type_macro_0027
    macro_0028 - road_type_macro_0028
    macro_0029 - road_type_macro_0029
    macro_0030 - road_type_macro_0030
    macro_0031 - road_type_macro_0031
    macro_0032 - road_type_macro_0032
    macro_0033 - road_type_macro_0033
    macro_0034 - road_type_macro_0034
    macro_0035 - road_type_macro_0035
    macro_0036 - road_type_macro_0036
    macro_0037 - road_type_macro_0037
    macro_0038 - road_type_macro_0038
    road_0000 - road_type_road_0000
    road_0001 - road_type_road_0001
    road_0002 - road_type_road_0002
    road_0003 - road_type_road_0003
    road_0004 - road_type_road_0004
    road_0005 - road_type_road_0005
    road_0006 - road_type_road_0006
    road_0007 - road_type_road_0007
    road_0008 - road_type_road_0008
    road_0009 - road_type_road_0009
    road_0010 - road_type_road_0010
    road_0012 - road_type_road_0012
    road_0013 - road_type_road_0013
    road_0014 - road_type_road_0014
    road_0015 - road_type_road_0015
    road_0016 - road_type_road_0016
    road_0017 - road_type_road_0017
    road_0019 - road_type_road_0019
    road_0020 - road_type_road_0020
    road_0021 - road_type_road_0021
    road_0022 - road_type_road_0022
    road_0023 - road_type_road_0023
    road_0026 - road_type_road_0026
    road_0027 - road_type_road_0027
    road_0028 - road_type_road_0028
    road_0030 - road_type_road_0030
    road_0031 - road_type_road_0031
    road_0032 - road_type_road_0032
    road_0033 - road_type_road_0033
    road_0034 - road_type_road_0034
    road_0035 - road_type_road_0035
    road_0036 - road_type_road_0036
    road_0037 - road_type_road_0037
    road_0038 - road_type_road_0038
    road_0039 - road_type_road_0039
    road_0040 - road_type_road_0040
    road_0041 - road_type_road_0041
    road_0042 - road_type_road_0042
    road_0043 - road_type_road_0043
    road_0044 - road_type_road_0044
    road_0045 - road_type_road_0045
    road_0046 - road_type_road_0046
    road_0047 - road_type_road_0047
    road_0048 - road_type_road_0048
    road_0049 - road_type_road_0049
    road_0050 - road_type_road_0050
    road_0051 - road_type_road_0051
    road_0052 - road_type_road_0052
    road_0053 - road_type_road_0053
    road_0058 - road_type_road_0058
    road_0059 - road_type_road_0059
    road_0061 - road_type_road_0061
    road_0062 - road_type_road_0062
    road_0063 - road_type_road_0063
    road_0064 - road_type_road_0064
    road_0065 - road_type_road_0065
    road_0066 - road_type_road_0066
    road_0067 - road_type_road_0067
    road_0068 - road_type_road_0068
    road_0069 - road_type_road_0069
    road_0070 - road_type_road_0070
    road_0071 - road_type_road_0071
    road_0072 - road_type_road_0072
    road_0073 - road_type_road_0073
    road_0074 - road_type_road_0074
    road_0075 - road_type_road_0075
    road_0076 - road_type_road_0076
    road_0077 - road_type_road_0077
    road_0078 - road_type_road_0078
    road_0079 - road_type_road_0079
    road_0080 - road_type_road_0080
    road_0081 - road_type_road_0081
    road_0082 - road_type_road_0082
    road_0083 - road_type_road_0083
    road_0084 - road_type_road_0084
    road_0085 - road_type_road_0085
    road_0086 - road_type_road_0086
    road_0087 - road_type_road_0087
    road_0088 - road_type_road_0088
    road_0090 - road_type_road_0090
    road_0094 - road_type_road_0094
    road_0095 - road_type_road_0095
    road_0096 - road_type_road_0096
    road_0097 - road_type_road_0097
    road_0099 - road_type_road_0099
    road_0100 - road_type_road_0100
    road_0101 - road_type_road_0101
    road_0102 - road_type_road_0102
    road_0103 - road_type_road_0103
    road_0104 - road_type_road_0104
    road_0105 - road_type_road_0105
    road_0107 - road_type_road_0107
    road_0108 - road_type_road_0108
    road_0109 - road_type_road_0109
    road_0110 - road_type_road_0110
    road_0111 - road_type_road_0111
    road_0112 - road_type_road_0112
    road_0113 - road_type_road_0113
    road_0114 - road_type_road_0114
    road_0115 - road_type_road_0115
    road_0116 - road_type_road_0116
    road_0117 - road_type_road_0117
    road_0118 - road_type_road_0118
    road_0120 - road_type_road_0120
    road_0121 - road_type_road_0121
    road_0122 - road_type_road_0122
    road_0123 - road_type_road_0123
    road_0124 - road_type_road_0124
    road_0125 - road_type_road_0125
    road_0126 - road_type_road_0126
    road_0127 - road_type_road_0127
    road_0128 - road_type_road_0128
    road_0129 - road_type_road_0129
    road_0130 - road_type_road_0130
    road_0131 - road_type_road_0131
    road_0132 - road_type_road_0132
    road_0133 - road_type_road_0133
    road_0134 - road_type_road_0134
    road_0135 - road_type_road_0135
    road_0137 - road_type_road_0137
    road_0138 - road_type_road_0138
    road_0139 - road_type_road_0139
    road_0140 - road_type_road_0140
    road_0141 - road_type_road_0141
    road_0142 - road_type_road_0142
    road_0143 - road_type_road_0143
    road_0144 - road_type_road_0144
    road_0145 - road_type_road_0145
    road_0146 - road_type_road_0146
    road_0147 - road_type_road_0147
    road_0148 - road_type_road_0148
    road_0149 - road_type_road_0149
    road_0150 - road_type_road_0150
    road_0151 - road_type_road_0151
    road_0152 - road_type_road_0152
    road_0153 - road_type_road_0153
    road_0155 - road_type_road_0155
    road_0157 - road_type_road_0157
    road_0158 - road_type_road_0158
    road_0159 - road_type_road_0159
    road_0161 - road_type_road_0161
    road_0162 - road_type_road_0162
    road_0163 - road_type_road_0163
    road_0164 - road_type_road_0164
    road_0165 - road_type_road_0165
    road_0166 - road_type_road_0166
    road_0167 - road_type_road_0167
    road_0168 - road_type_road_0168
    road_0169 - road_type_road_0169
    road_0171 - road_type_road_0171
    road_0172 - road_type_road_0172
    road_0173 - road_type_road_0173
    road_0174 - road_type_road_0174
    road_0175 - road_type_road_0175
    road_0176 - road_type_road_0176
    road_0177 - road_type_road_0177
    road_0178 - road_type_road_0178
    road_0179 - road_type_road_0179
    road_0180 - road_type_road_0180
    road_0181 - road_type_road_0181
    road_0182 - road_type_road_0182
    road_0183 - road_type_road_0183
    road_0184 - road_type_road_0184
    road_0185 - road_type_road_0185
    road_0186 - road_type_road_0186
    road_0187 - road_type_road_0187
    road_0188 - road_type_road_0188
    road_0189 - road_type_road_0189
    road_0190 - road_type_road_0190
    road_0191 - road_type_road_0191
    road_0192 - road_type_road_0192
    road_0193 - road_type_road_0193
    road_0194 - road_type_road_0194
    road_0195 - road_type_road_0195
    road_0196 - road_type_road_0196
    road_0197 - road_type_road_0197
    road_0198 - road_type_road_0198
    road_0199 - road_type_road_0199
    road_0200 - road_type_road_0200
    road_0201 - road_type_road_0201
    road_0202 - road_type_road_0202
    road_0203 - road_type_road_0203
    road_0204 - road_type_road_0204
    road_0205 - road_type_road_0205
    road_0206 - road_type_road_0206
    road_0207 - road_type_road_0207
    road_0208 - road_type_road_0208
    road_0209 - road_type_road_0209
    road_0210 - road_type_road_0210
    road_0211 - road_type_road_0211
    road_0212 - road_type_road_0212
    road_0213 - road_type_road_0213
    road_0215 - road_type_road_0215
    road_0216 - road_type_road_0216
    road_0217 - road_type_road_0217
    road_0218 - road_type_road_0218
    road_0219 - road_type_road_0219
    road_0220 - road_type_road_0220
    road_0221 - road_type_road_0221
    road_0222 - road_type_road_0222
    road_0223 - road_type_road_0223
    road_0226 - road_type_road_0226
    road_0227 - road_type_road_0227
    road_0228 - road_type_road_0228
    road_0232 - road_type_road_0232
    road_0233 - road_type_road_0233
    road_0234 - road_type_road_0234
    road_0235 - road_type_road_0235
    road_0236 - road_type_road_0236
    road_0237 - road_type_road_0237
    road_0238 - road_type_road_0238
    road_0239 - road_type_road_0239
    road_0240 - road_type_road_0240
    road_0241 - road_type_road_0241
    road_0242 - road_type_road_0242
    road_0243 - road_type_road_0243
    road_0244 - road_type_road_0244
    road_0245 - road_type_road_0245
    road_0246 - road_type_road_0246
    road_0247 - road_type_road_0247
    road_0248 - road_type_road_0248
    road_0249 - road_type_road_0249
    road_0250 - road_type_road_0250
    road_0251 - road_type_road_0251
    road_0252 - road_type_road_0252
    road_0253 - road_type_road_0253
    road_0254 - road_type_road_0254
    road_0255 - road_type_road_0255
    road_0256 - road_type_road_0256
    road_0257 - road_type_road_0257
    road_0258 - road_type_road_0258
    road_0259 - road_type_road_0259
    road_0260 - road_type_road_0260
    road_0261 - road_type_road_0261
    road_0262 - road_type_road_0262
    road_0263 - road_type_road_0263
    road_0264 - road_type_road_0264
    road_0265 - road_type_road_0265
    road_0266 - road_type_road_0266
    road_0269 - road_type_road_0269
    road_0270 - road_type_road_0270
    road_0272 - road_type_road_0272
    road_0273 - road_type_road_0273
    road_0274 - road_type_road_0274
    road_0275 - road_type_road_0275
    road_0276 - road_type_road_0276
    road_0277 - road_type_road_0277
    road_0278 - road_type_road_0278
    road_0279 - road_type_road_0279
    road_0280 - road_type_road_0280
    road_0281 - road_type_road_0281
    road_0282 - road_type_road_0282
    road_0283 - road_type_road_0283
    road_0285 - road_type_road_0285
    road_0286 - road_type_road_0286
    road_0287 - road_type_road_0287
    road_0288 - road_type_road_0288
    road_0289 - road_type_road_0289
    road_0290 - road_type_road_0290
    road_0291 - road_type_road_0291
    road_0292 - road_type_road_0292
    road_0293 - road_type_road_0293
    road_0294 - road_type_road_0294
    road_0295 - road_type_road_0295
    road_0297 - road_type_road_0297
    road_0298 - road_type_road_0298
    road_0299 - road_type_road_0299
    road_0300 - road_type_road_0300
    road_0301 - road_type_road_0301
    road_0302 - road_type_road_0302
    road_0303 - road_type_road_0303
    road_0304 - road_type_road_0304
    road_0305 - road_type_road_0305
    road_0306 - road_type_road_0306
    road_0307 - road_type_road_0307
    road_0309 - road_type_road_0309
    road_0310 - road_type_road_0310
    road_0311 - road_type_road_0311
    road_0312 - road_type_road_0312
    road_0313 - road_type_road_0313
    road_0314 - road_type_road_0314
    road_0315 - road_type_road_0315
    road_0316 - road_type_road_0316
    road_0317 - road_type_road_0317
    road_0318 - road_type_road_0318
    road_0321 - road_type_road_0321
    road_0323 - road_type_road_0323
    road_0324 - road_type_road_0324
    road_0325 - road_type_road_0325
    road_0327 - road_type_road_0327
    road_0328 - road_type_road_0328
    road_0329 - road_type_road_0329
    road_0330 - road_type_road_0330
    road_0331 - road_type_road_0331
    road_0332 - road_type_road_0332
    road_0333 - road_type_road_0333
    road_0334 - road_type_road_0334
    road_0335 - road_type_road_0335
    road_0336 - road_type_road_0336
    road_0337 - road_type_road_0337
    road_0338 - road_type_road_0338
    road_0339 - road_type_road_0339
    road_0340 - road_type_road_0340
    road_0341 - road_type_road_0341
    road_0342 - road_type_road_0342
    road_0344 - road_type_road_0344
    road_0345 - road_type_road_0345
    road_0346 - road_type_road_0346
    road_0347 - road_type_road_0347
    road_0348 - road_type_road_0348
    road_0349 - road_type_road_0349
    road_0350 - road_type_road_0350
    road_0351 - road_type_road_0351
    road_0352 - road_type_road_0352
    road_0353 - road_type_road_0353
    road_0354 - road_type_road_0354
    road_0355 - road_type_road_0355
    road_0356 - road_type_road_0356
    road_0357 - road_type_road_0357
    road_0358 - road_type_road_0358
    road_0359 - road_type_road_0359
    road_0360 - road_type_road_0360
    road_0361 - road_type_road_0361
    road_0364 - road_type_road_0364
    road_0366 - road_type_road_0366
    road_0367 - road_type_road_0367
    road_0368 - road_type_road_0368
    road_0369 - road_type_road_0369
    road_0370 - road_type_road_0370
    road_0371 - road_type_road_0371
    road_0372 - road_type_road_0372
    road_0373 - road_type_road_0373
    road_0374 - road_type_road_0374
    road_0375 - road_type_road_0375
    road_0376 - road_type_road_0376
    road_0377 - road_type_road_0377
    road_0379 - road_type_road_0379
    road_0380 - road_type_road_0380
    road_0381 - road_type_road_0381
    road_0382 - road_type_road_0382
    road_0383 - road_type_road_0383
    road_0384 - road_type_road_0384
    road_0385 - road_type_road_0385
    road_0386 - road_type_road_0386
    road_0387 - road_type_road_0387
    road_0388 - road_type_road_0388
    road_0390 - road_type_road_0390
    road_0391 - road_type_road_0391
    road_0392 - road_type_road_0392
    road_0393 - road_type_road_0393
    road_0395 - road_type_road_0395
    road_0396 - road_type_road_0396
    road_0397 - road_type_road_0397
    road_0398 - road_type_road_0398
    road_0400 - road_type_road_0400
    road_0401 - road_type_road_0401
    road_0402 - road_type_road_0402
    road_0403 - road_type_road_0403
    road_0404 - road_type_road_0404
    road_0405 - road_type_road_0405
    road_0406 - road_type_road_0406
    road_0407 - road_type_road_0407
    road_0408 - road_type_road_0408
    road_0409 - road_type_road_0409
    road_0410 - road_type_road_0410
    road_0411 - road_type_road_0411
    road_0412 - road_type_road_0412
    road_0414 - road_type_road_0414
    road_0415 - road_type_road_0415
    road_0416 - road_type_road_0416
    road_0417 - road_type_road_0417
    road_0419 - road_type_road_0419
    road_0420 - road_type_road_0420
    road_0422 - road_type_road_0422
    road_0423 - road_type_road_0423
    road_0424 - road_type_road_0424
    road_0425 - road_type_road_0425
    road_0426 - road_type_road_0426
    road_0430 - road_type_road_0430
    road_0431 - road_type_road_0431
    road_0432 - road_type_road_0432
    road_0433 - road_type_road_0433
    road_0435 - road_type_road_0435
    road_0437 - road_type_road_0437
    road_0438 - road_type_road_0438
    road_0439 - road_type_road_0439
    road_0440 - road_type_road_0440
    road_0441 - road_type_road_0441
    road_0442 - road_type_road_0442
    road_0443 - road_type_road_0443
    road_0444 - road_type_road_0444
    road_0445 - road_type_road_0445
    road_0446 - road_type_road_0446
    road_0447 - road_type_road_0447
    road_0448 - road_type_road_0448
    road_0449 - road_type_road_0449
    road_0450 - road_type_road_0450
    road_0451 - road_type_road_0451
    road_0452 - road_type_road_0452
    road_0453 - road_type_road_0453
    road_0454 - road_type_road_0454
    road_0455 - road_type_road_0455
    road_0456 - road_type_road_0456
    road_0457 - road_type_road_0457
    road_0458 - road_type_road_0458
    road_0459 - road_type_road_0459
    road_0460 - road_type_road_0460
    road_0461 - road_type_road_0461
    road_0462 - road_type_road_0462
    road_0463 - road_type_road_0463
    road_0464 - road_type_road_0464
    road_0465 - road_type_road_0465
    road_0466 - road_type_road_0466
    road_0467 - road_type_road_0467
    road_0468 - road_type_road_0468
    road_0470 - road_type_road_0470
    road_0472 - road_type_road_0472
    road_0473 - road_type_road_0473
    road_0474 - road_type_road_0474
    road_0475 - road_type_road_0475
    road_0476 - road_type_road_0476
    road_0478 - road_type_road_0478
    road_0479 - road_type_road_0479
    road_0480 - road_type_road_0480
    road_0481 - road_type_road_0481
    road_0482 - road_type_road_0482
    road_0483 - road_type_road_0483
    road_0484 - road_type_road_0484
    road_0485 - road_type_road_0485
    road_0486 - road_type_road_0486
    road_0487 - road_type_road_0487
    road_0488 - road_type_road_0488
    road_0489 - road_type_road_0489
    road_0490 - road_type_road_0490
    road_0493 - road_type_road_0493
    road_0494 - road_type_road_0494
    road_0495 - road_type_road_0495
    road_0496 - road_type_road_0496
    road_0497 - road_type_road_0497
    road_0498 - road_type_road_0498
    road_0499 - road_type_road_0499
    road_0500 - road_type_road_0500
    road_0501 - road_type_road_0501
    road_0502 - road_type_road_0502
    road_0503 - road_type_road_0503
  )

  (:init
  (ready-road car1 road_0325)
  (= (travel-time car1) 0)
  (connects macro_0000 loc_0008 loc_0114)
  (road-open macro_0000)
  (= (congestion-factor macro_0000) 1.05)
  (= (travel-duration macro_0000) 8.5784)
  (road-next macro_0000 road_0195)
  (road-next macro_0000 road_0196)
  (connects macro_0001 loc_0012 loc_0161)
  (road-open macro_0001)
  (= (congestion-factor macro_0001) 1.17)
  (= (travel-duration macro_0001) 22.7513)
  (road-next macro_0001 macro_0026)
  (road-next macro_0001 road_0283)
  (road-next macro_0001 road_0285)
  (connects macro_0002 loc_0016 loc_0285)
  (road-open macro_0002)
  (= (congestion-factor macro_0002) 1.3)
  (= (travel-duration macro_0002) 34.6646)
  (road-next macro_0002 road_0478)
  (road-next macro_0002 road_0479)
  (connects macro_0003 loc_0016 loc_0188)
  (road-open macro_0003)
  (= (congestion-factor macro_0003) 1.05)
  (= (travel-duration macro_0003) 14.1113)
  (road-next macro_0003 road_0323)
  (road-next macro_0003 road_0324)
  (connects macro_0004 loc_0021 loc_0022)
  (road-open macro_0004)
  (= (congestion-factor macro_0004) 1.06)
  (= (travel-duration macro_0004) 11.0389)
  (road-next macro_0004 road_0031)
  (connects macro_0005 loc_0034 loc_0035)
  (road-open macro_0005)
  (= (congestion-factor macro_0005) 1.1)
  (= (travel-duration macro_0005) 16.2815)
  (road-next macro_0005 macro_0006)
  (road-next macro_0005 macro_0007)
  (connects macro_0006 loc_0035 loc_0296)
  (road-open macro_0006)
  (= (congestion-factor macro_0006) 1.05)
  (= (travel-duration macro_0006) 40.0822)
  (road-next macro_0006 road_0498)
  (connects macro_0007 loc_0035 loc_0156)
  (road-open macro_0007)
  (= (congestion-factor macro_0007) 1.0)
  (= (travel-duration macro_0007) 16.7011)
  (road-next macro_0007 road_0276)
  (connects macro_0008 loc_0038 loc_0280)
  (road-open macro_0008)
  (= (congestion-factor macro_0008) 1.17)
  (= (travel-duration macro_0008) 14.0321)
  (road-next macro_0008 macro_0036)
  (road-next macro_0008 road_0472)
  (connects macro_0009 loc_0052 loc_0267)
  (road-open macro_0009)
  ;; [DYNAMIC EVENT - location closure] loc_0267 incident road
  (road-blocked macro_0009)
  (= (congestion-factor macro_0009) 1.0)
  (= (travel-duration macro_0009) 16.0326)
  (road-next macro_0009 road_0453)
  (road-next macro_0009 road_0454)
  (connects macro_0010 loc_0053 loc_0071)
  (road-open macro_0010)
  (= (congestion-factor macro_0010) 1.0)
  (= (travel-duration macro_0010) 72.7649)
  (road-next macro_0010 road_0118)
  (connects macro_0011 loc_0054 loc_0127)
  (road-open macro_0011)
  (= (congestion-factor macro_0011) 1.05)
  (= (travel-duration macro_0011) 20.5098)
  (road-next macro_0011 road_0218)
  (connects macro_0012 loc_0054 loc_0004)
  (road-open macro_0012)
  (= (congestion-factor macro_0012) 1.03)
  (= (travel-duration macro_0012) 23.0795)
  (road-next macro_0012 road_0006)
  (road-next macro_0012 road_0007)
  (connects macro_0013 loc_0057 loc_0064)
  (road-open macro_0013)
  (= (congestion-factor macro_0013) 1.0)
  (= (travel-duration macro_0013) 18.4723)
  (road-next macro_0013 road_0107)
  (connects macro_0014 loc_0063 loc_0166)
  (road-open macro_0014)
  (= (congestion-factor macro_0014) 1.2)
  (= (travel-duration macro_0014) 23.0683)
  (road-next macro_0014 road_0294)
  (connects macro_0015 loc_0084 loc_0249)
  (road-open macro_0015)
  (= (congestion-factor macro_0015) 1.06)
  (= (travel-duration macro_0015) 30.6815)
  (road-next macro_0015 road_0425)
  (connects macro_0016 loc_0095 loc_0166)
  (road-open macro_0016)
  (= (congestion-factor macro_0016) 1.1)
  (= (travel-duration macro_0016) 31.5862)
  (road-next macro_0016 road_0294)
  (connects macro_0017 loc_0096 loc_0166)
  (road-open macro_0017)
  (= (congestion-factor macro_0017) 1.1)
  (= (travel-duration macro_0017) 43.8007)
  (road-next macro_0017 road_0294)
  (connects macro_0018 loc_0098 loc_0099)
  (road-open macro_0018)
  (= (congestion-factor macro_0018) 1.02)
  (= (travel-duration macro_0018) 1.717)
  (road-next macro_0018 road_0161)
  (road-next macro_0018 road_0162)
  (connects macro_0019 loc_0102 loc_0293)
  (road-open macro_0019)
  (= (congestion-factor macro_0019) 1.15)
  (= (travel-duration macro_0019) 15.9783)
  (road-next macro_0019 macro_0037)
  (road-next macro_0019 macro_0038)
  (connects macro_0020 loc_0125 loc_0283)
  (road-open macro_0020)
  (= (congestion-factor macro_0020) 1.05)
  (= (travel-duration macro_0020) 25.0926)
  (road-next macro_0020 road_0474)
  (road-next macro_0020 road_0475)
  (road-next macro_0020 road_0476)
  (connects macro_0021 loc_0131 loc_0130)
  (road-open macro_0021)
  (= (congestion-factor macro_0021) 1.0)
  (= (travel-duration macro_0021) 37.2543)
  (road-next macro_0021 road_0221)
  (road-next macro_0021 road_0222)
  (road-next macro_0021 road_0223)
  (connects macro_0022 loc_0131 loc_0130)
  (road-open macro_0022)
  (= (congestion-factor macro_0022) 1.1)
  (= (travel-duration macro_0022) 40.9507)
  (road-next macro_0022 road_0221)
  (road-next macro_0022 road_0222)
  (road-next macro_0022 road_0223)
  (connects macro_0023 loc_0133 loc_0132)
  (road-open macro_0023)
  (= (congestion-factor macro_0023) 1.05)
  (= (travel-duration macro_0023) 45.7575)
  (road-next macro_0023 road_0227)
  (road-next macro_0023 road_0228)
  (connects macro_0024 loc_0133 loc_0132)
  (road-open macro_0024)
  (= (congestion-factor macro_0024) 1.1)
  (= (travel-duration macro_0024) 47.9074)
  (road-next macro_0024 road_0227)
  (road-next macro_0024 road_0228)
  (connects macro_0025 loc_0154 loc_0157)
  (road-open macro_0025)
  (= (congestion-factor macro_0025) 1.05)
  (= (travel-duration macro_0025) 21.7712)
  (road-next macro_0025 road_0277)
  (connects macro_0026 loc_0161 loc_0012)
  (road-open macro_0026)
  (= (congestion-factor macro_0026) 1.06)
  (= (travel-duration macro_0026) 27.2823)
  (road-next macro_0026 macro_0001)
  (road-next macro_0026 road_0016)
  (road-next macro_0026 road_0017)
  (connects macro_0027 loc_0176 loc_0103)
  (road-open macro_0027)
  (= (congestion-factor macro_0027) 1.0)
  (= (travel-duration macro_0027) 15.6018)
  (road-next macro_0027 road_0171)
  (road-next macro_0027 road_0172)
  (connects macro_0028 loc_0184 loc_0068)
  (road-open macro_0028)
  (= (congestion-factor macro_0028) 1.14)
  (= (travel-duration macro_0028) 10.8199)
  (road-next macro_0028 road_0113)
  (road-next macro_0028 road_0114)
  (connects macro_0029 loc_0187 loc_0127)
  (road-open macro_0029)
  (= (congestion-factor macro_0029) 1.14)
  (= (travel-duration macro_0029) 20.5167)
  (road-next macro_0029 road_0218)
  (connects macro_0030 loc_0218 loc_0153)
  (road-open macro_0030)
  (= (congestion-factor macro_0030) 1.15)
  (= (travel-duration macro_0030) 17.3666)
  (road-next macro_0030 road_0270)
  (connects macro_0031 loc_0229 loc_0099)
  (road-open macro_0031)
  (= (congestion-factor macro_0031) 1.08)
  (= (travel-duration macro_0031) 5.926)
  (road-next macro_0031 road_0161)
  (road-next macro_0031 road_0162)
  (connects macro_0032 loc_0233 loc_0141)
  (road-open macro_0032)
  (= (congestion-factor macro_0032) 1.0)
  (= (travel-duration macro_0032) 47.7019)
  (road-next macro_0032 road_0250)
  (road-next macro_0032 road_0251)
  (connects macro_0033 loc_0241 loc_0114)
  (road-open macro_0033)
  (= (congestion-factor macro_0033) 1.2)
  (= (travel-duration macro_0033) 36.4248)
  (road-next macro_0033 road_0195)
  (road-next macro_0033 road_0196)
  (connects macro_0034 loc_0244 loc_0245)
  (road-open macro_0034)
  (= (congestion-factor macro_0034) 1.2)
  (= (travel-duration macro_0034) 2.15)
  (road-next macro_0034 road_0419)
  (road-next macro_0034 road_0420)
  (connects macro_0035 loc_0255 loc_0164)
  (road-open macro_0035)
  (= (congestion-factor macro_0035) 1.05)
  (= (travel-duration macro_0035) 23.6637)
  (road-next macro_0035 road_0290)
  (road-next macro_0035 road_0291)
  (connects macro_0036 loc_0280 loc_0038)
  (road-open macro_0036)
  (= (congestion-factor macro_0036) 1.03)
  (= (travel-duration macro_0036) 12.4062)
  (road-next macro_0036 macro_0008)
  (road-next macro_0036 road_0059)
  (road-next macro_0036 road_0061)
  (connects macro_0037 loc_0293 loc_0102)
  (road-open macro_0037)
  (= (congestion-factor macro_0037) 1.0)
  (= (travel-duration macro_0037) 13.8942)
  (road-next macro_0037 macro_0019)
  (road-next macro_0037 road_0168)
  (road-next macro_0037 road_0169)
  (connects macro_0038 loc_0293 loc_0201)
  (road-open macro_0038)
  (= (congestion-factor macro_0038) 1.4)
  (= (travel-duration macro_0038) 45.172)
  (road-next macro_0038 road_0347)
  (road-next macro_0038 road_0348)
  (connects road_0000 loc_0000 loc_0215)
  (road-open road_0000)
  (= (congestion-factor road_0000) 1.03)
  (= (travel-duration road_0000) 14.1664)
  (road-next road_0000 road_0375)
  (connects road_0001 loc_0000 loc_0050)
  (road-open road_0001)
  (= (congestion-factor road_0001) 1.06)
  (= (travel-duration road_0001) 7.1616)
  (road-next road_0001 road_0084)
  (road-next road_0001 road_0085)
  (connects road_0002 loc_0000 loc_0029)
  (road-open road_0002)
  (= (congestion-factor road_0002) 1.0)
  (= (travel-duration road_0002) 10.5532)
  (road-next road_0002 road_0044)
  (road-next road_0002 road_0045)
  (connects road_0003 loc_0001 loc_0120)
  (road-open road_0003)
  (= (congestion-factor road_0003) 1.2)
  (= (travel-duration road_0003) 8.8967)
  (road-next road_0003 road_0206)
  (connects road_0004 loc_0001 loc_0056)
  (road-open road_0004)
  (= (congestion-factor road_0004) 1.09)
  (= (travel-duration road_0004) 24.2225)
  (road-next road_0004 road_0096)
  (connects road_0005 loc_0002 loc_0188)
  (road-open road_0005)
  (= (congestion-factor road_0005) 1.0)
  (= (travel-duration road_0005) 1.6957)
  (road-next road_0005 road_0323)
  (road-next road_0005 road_0324)
  (connects road_0006 loc_0004 loc_0216)
  (road-open road_0006)
  (= (congestion-factor road_0006) 1.1)
  (= (travel-duration road_0006) 100.2513)
  (road-next road_0006 road_0376)
  (connects road_0007 loc_0004 loc_0046)
  (road-open road_0007)
  (= (congestion-factor road_0007) 1.0)
  (= (travel-duration road_0007) 3.0661)
  (road-next road_0007 road_0078)
  (road-next road_0007 road_0079)
  (connects road_0008 loc_0005 loc_0085)
  (road-open road_0008)
  (= (congestion-factor road_0008) 1.05)
  (= (travel-duration road_0008) 14.8472)
  (road-next road_0008 road_0137)
  (connects road_0009 loc_0006 loc_0127)
  (road-open road_0009)
  (= (congestion-factor road_0009) 1.0)
  (= (travel-duration road_0009) 8.3307)
  (road-next road_0009 road_0218)
  (connects road_0010 loc_0007 loc_0114)
  (road-open road_0010)
  (= (congestion-factor road_0010) 1.05)
  (= (travel-duration road_0010) 4.4442)
  (road-next road_0010 road_0195)
  (road-next road_0010 road_0196)
  (connects road_0012 loc_0009 loc_0157)
  (road-open road_0012)
  (= (congestion-factor road_0012) 1.0)
  (= (travel-duration road_0012) 12.0833)
  (road-next road_0012 road_0277)
  (connects road_0013 loc_0010 loc_0008)
  (road-open road_0013)
  (= (congestion-factor road_0013) 1.05)
  (= (travel-duration road_0013) 8.5998)
  (road-next road_0013 macro_0000)
  (connects road_0014 loc_0010 loc_0115)
  (road-open road_0014)
  (= (congestion-factor road_0014) 1.25)
  (= (travel-duration road_0014) 17.6272)
  (road-next road_0014 road_0197)
  (connects road_0015 loc_0011 loc_0241)
  (road-open road_0015)
  (= (congestion-factor road_0015) 1.55)
  (= (travel-duration road_0015) 13.6306)
  (road-next road_0015 macro_0033)
  (road-next road_0015 road_0414)
  (connects road_0016 loc_0012 loc_0163)
  (road-open road_0016)
  (= (congestion-factor road_0016) 1.06)
  (= (travel-duration road_0016) 9.7096)
  (road-next road_0016 road_0287)
  (road-next road_0016 road_0288)
  (road-next road_0016 road_0289)
  (connects road_0017 loc_0012 loc_0094)
  (road-open road_0017)
  ;; [DYNAMIC EVENT - location closure] loc_0094 incident road
  (road-blocked road_0017)
  (= (congestion-factor road_0017) 1.0)
  (= (travel-duration road_0017) 10.2232)
  (road-next road_0017 road_0151)
  (road-next road_0017 road_0152)
  (road-next road_0017 road_0153)
  (connects road_0019 loc_0013 loc_0135)
  (road-open road_0019)
  (= (congestion-factor road_0019) 1.1)
  (= (travel-duration road_0019) 23.2884)
  (road-next road_0019 road_0235)
  (road-next road_0019 road_0236)
  (road-next road_0019 road_0237)
  (connects road_0020 loc_0013 loc_0017)
  (road-open road_0020)
  (= (congestion-factor road_0020) 1.1)
  (= (travel-duration road_0020) 2.1279)
  (road-next road_0020 road_0026)
  (connects road_0021 loc_0013 loc_0237)
  (road-open road_0021)
  (= (congestion-factor road_0021) 1.04)
  (= (travel-duration road_0021) 6.1296)
  (road-next road_0021 road_0405)
  (road-next road_0021 road_0406)
  (connects road_0022 loc_0014 loc_0088)
  (road-open road_0022)
  (= (congestion-factor road_0022) 1.0)
  (= (travel-duration road_0022) 2.0893)
  (road-next road_0022 road_0141)
  (road-next road_0022 road_0142)
  (connects road_0023 loc_0014 loc_0015)
  (road-open road_0023)
  (= (congestion-factor road_0023) 1.04)
  (= (travel-duration road_0023) 10.7437)
  (connects road_0026 loc_0017 loc_0013)
  (road-open road_0026)
  (= (congestion-factor road_0026) 1.2)
  (= (travel-duration road_0026) 2.3214)
  (road-next road_0026 road_0019)
  (road-next road_0026 road_0020)
  (road-next road_0026 road_0021)
  (connects road_0027 loc_0019 loc_0221)
  (road-open road_0027)
  (= (congestion-factor road_0027) 1.23)
  (= (travel-duration road_0027) 0.8679)
  (road-next road_0027 road_0382)
  (road-next road_0027 road_0383)
  (connects road_0028 loc_0020 loc_0222)
  (road-open road_0028)
  (= (congestion-factor road_0028) 1.31)
  (= (travel-duration road_0028) 7.7204)
  (road-next road_0028 road_0384)
  (connects road_0030 loc_0021 loc_0022)
  (road-open road_0030)
  (= (congestion-factor road_0030) 1.2)
  (= (travel-duration road_0030) 6.3507)
  (road-next road_0030 road_0031)
  (connects road_0031 loc_0022 loc_0224)
  (road-open road_0031)
  (= (congestion-factor road_0031) 1.26)
  (= (travel-duration road_0031) 1.2081)
  (road-next road_0031 road_0387)
  (road-next road_0031 road_0388)
  (connects road_0032 loc_0023 loc_0254)
  (road-open road_0032)
  (= (congestion-factor road_0032) 1.05)
  (= (travel-duration road_0032) 17.7453)
  (road-next road_0032 road_0432)
  (road-next road_0032 road_0433)
  (connects road_0033 loc_0024 loc_0017)
  (road-open road_0033)
  (= (congestion-factor road_0033) 1.2)
  (= (travel-duration road_0033) 20.1248)
  (road-next road_0033 road_0026)
  (connects road_0034 loc_0024 loc_0097)
  (road-open road_0034)
  (= (congestion-factor road_0034) 1.17)
  (= (travel-duration road_0034) 20.0949)
  (road-next road_0034 road_0157)
  (road-next road_0034 road_0158)
  (road-next road_0034 road_0159)
  (connects road_0035 loc_0024 loc_0060)
  (road-open road_0035)
  (= (congestion-factor road_0035) 1.09)
  (= (travel-duration road_0035) 3.8444)
  (road-next road_0035 road_0102)
  (road-next road_0035 road_0103)
  (connects road_0036 loc_0025 loc_0147)
  (road-open road_0036)
  (= (congestion-factor road_0036) 1.17)
  (= (travel-duration road_0036) 7.0385)
  (road-next road_0036 road_0260)
  (road-next road_0036 road_0261)
  (road-next road_0036 road_0262)
  (connects road_0037 loc_0025 loc_0026)
  (road-open road_0037)
  (= (congestion-factor road_0037) 1.23)
  (= (travel-duration road_0037) 10.1036)
  (road-next road_0037 road_0038)
  (road-next road_0037 road_0039)
  (road-next road_0037 road_0040)
  (connects road_0038 loc_0026 loc_0090)
  (road-open road_0038)
  (= (congestion-factor road_0038) 1.2)
  (= (travel-duration road_0038) 52.5702)
  (road-next road_0038 road_0145)
  (connects road_0039 loc_0026 loc_0025)
  (road-open road_0039)
  (= (congestion-factor road_0039) 1.09)
  (= (travel-duration road_0039) 8.9536)
  (road-next road_0039 road_0036)
  (road-next road_0039 road_0037)
  (connects road_0040 loc_0026 loc_0273)
  (road-open road_0040)
  (= (congestion-factor road_0040) 1.17)
  (= (travel-duration road_0040) 5.3452)
  (road-next road_0040 road_0462)
  (road-next road_0040 road_0463)
  (connects road_0041 loc_0027 loc_0219)
  (road-open road_0041)
  (= (congestion-factor road_0041) 1.15)
  (= (travel-duration road_0041) 1.2503)
  (road-next road_0041 road_0379)
  (connects road_0042 loc_0027 loc_0286)
  (road-open road_0042)
  (= (congestion-factor road_0042) 1.15)
  (= (travel-duration road_0042) 0.563)
  (road-next road_0042 road_0480)
  (connects road_0043 loc_0028 loc_0164)
  (road-open road_0043)
  (= (congestion-factor road_0043) 1.05)
  (= (travel-duration road_0043) 6.9731)
  (road-next road_0043 road_0290)
  (road-next road_0043 road_0291)
  (connects road_0044 loc_0029 loc_0107)
  (road-open road_0044)
  (= (congestion-factor road_0044) 1.0)
  (= (travel-duration road_0044) 42.9485)
  (road-next road_0044 road_0181)
  (road-next road_0044 road_0182)
  (road-next road_0044 road_0183)
  (connects road_0045 loc_0029 loc_0000)
  (road-open road_0045)
  (= (congestion-factor road_0045) 1.03)
  (= (travel-duration road_0045) 10.8698)
  (road-next road_0045 road_0000)
  (road-next road_0045 road_0001)
  (road-next road_0045 road_0002)
  (connects road_0046 loc_0030 loc_0010)
  (road-open road_0046)
  (= (congestion-factor road_0046) 1.15)
  (= (travel-duration road_0046) 13.0001)
  (road-next road_0046 road_0013)
  (road-next road_0046 road_0014)
  (connects road_0047 loc_0030 loc_0146)
  (road-open road_0047)
  (= (congestion-factor road_0047) 1.0)
  (= (travel-duration road_0047) 8.3463)
  (road-next road_0047 road_0257)
  (road-next road_0047 road_0258)
  (road-next road_0047 road_0259)
  (connects road_0048 loc_0031 loc_0154)
  (road-open road_0048)
  (= (congestion-factor road_0048) 1.05)
  (= (travel-duration road_0048) 12.2527)
  (road-next road_0048 macro_0025)
  (road-next road_0048 road_0272)
  (connects road_0049 loc_0031 loc_0032)
  (road-open road_0049)
  (= (congestion-factor road_0049) 1.15)
  (= (travel-duration road_0049) 4.7819)
  (road-next road_0049 road_0050)
  (road-next road_0049 road_0051)
  (connects road_0050 loc_0032 loc_0218)
  (road-open road_0050)
  (= (congestion-factor road_0050) 1.05)
  (= (travel-duration road_0050) 9.0283)
  (road-next road_0050 macro_0030)
  (connects road_0051 loc_0032 loc_0030)
  (road-open road_0051)
  (= (congestion-factor road_0051) 1.2)
  (= (travel-duration road_0051) 9.8673)
  (road-next road_0051 road_0046)
  (road-next road_0051 road_0047)
  (connects road_0052 loc_0033 loc_0039)
  (road-open road_0052)
  (= (congestion-factor road_0052) 1.14)
  (= (travel-duration road_0052) 14.1635)
  (road-next road_0052 road_0062)
  (road-next road_0052 road_0063)
  (connects road_0053 loc_0034 loc_0033)
  (road-open road_0053)
  (= (congestion-factor road_0053) 1.2)
  (= (travel-duration road_0053) 44.1968)
  (road-next road_0053 road_0052)
  (connects road_0058 loc_0037 loc_0223)
  (road-open road_0058)
  (= (congestion-factor road_0058) 1.11)
  (= (travel-duration road_0058) 16.2138)
  (road-next road_0058 road_0385)
  (road-next road_0058 road_0386)
  (connects road_0059 loc_0038 loc_0220)
  (road-open road_0059)
  (= (congestion-factor road_0059) 1.11)
  (= (travel-duration road_0059) 1.1069)
  (road-next road_0059 road_0380)
  (road-next road_0059 road_0381)
  (connects road_0061 loc_0038 loc_0037)
  (road-open road_0061)
  (= (congestion-factor road_0061) 1.25)
  (= (travel-duration road_0061) 37.1745)
  (road-next road_0061 road_0058)
  (connects road_0062 loc_0039 loc_0044)
  (road-open road_0062)
  (= (congestion-factor road_0062) 1.25)
  (= (travel-duration road_0062) 28.6676)
  (road-next road_0062 road_0074)
  (connects road_0063 loc_0039 loc_0222)
  (road-open road_0063)
  (= (congestion-factor road_0063) 1.03)
  (= (travel-duration road_0063) 20.4739)
  (road-next road_0063 road_0384)
  (connects road_0064 loc_0040 loc_0038)
  (road-open road_0064)
  (= (congestion-factor road_0064) 1.4)
  (= (travel-duration road_0064) 12.1284)
  (road-next road_0064 macro_0008)
  (road-next road_0064 road_0059)
  (road-next road_0064 road_0061)
  (connects road_0065 loc_0041 loc_0211)
  (road-open road_0065)
  (= (congestion-factor road_0065) 1.29)
  (= (travel-duration road_0065) 17.5488)
  (road-next road_0065 road_0368)
  (road-next road_0065 road_0369)
  (road-next road_0065 road_0370)
  (connects road_0066 loc_0042 loc_0043)
  (road-open road_0066)
  (= (congestion-factor road_0066) 1.2)
  (= (travel-duration road_0066) 9.9479)
  (road-next road_0066 road_0070)
  (road-next road_0066 road_0071)
  (road-next road_0066 road_0072)
  (road-next road_0066 road_0073)
  (connects road_0067 loc_0042 loc_0178)
  (road-open road_0067)
  (= (congestion-factor road_0067) 1.06)
  (= (travel-duration road_0067) 4.5272)
  (road-next road_0067 road_0311)
  (road-next road_0067 road_0312)
  (connects road_0068 loc_0042 loc_0266)
  (road-open road_0068)
  (= (congestion-factor road_0068) 1.0)
  (= (travel-duration road_0068) 7.0227)
  (road-next road_0068 road_0451)
  (road-next road_0068 road_0452)
  (connects road_0069 loc_0042 loc_0077)
  (road-open road_0069)
  (= (congestion-factor road_0069) 1.11)
  (= (travel-duration road_0069) 6.2926)
  (road-next road_0069 road_0124)
  (connects road_0070 loc_0043 loc_0177)
  (road-open road_0070)
  (= (congestion-factor road_0070) 1.15)
  (= (travel-duration road_0070) 9.8081)
  (road-next road_0070 road_0309)
  (road-next road_0070 road_0310)
  (connects road_0071 loc_0043 loc_0078)
  (road-open road_0071)
  (= (congestion-factor road_0071) 1.1)
  (= (travel-duration road_0071) 12.4613)
  (road-next road_0071 road_0125)
  (connects road_0072 loc_0043 loc_0045)
  (road-open road_0072)
  (= (congestion-factor road_0072) 1.15)
  (= (travel-duration road_0072) 5.3643)
  (road-next road_0072 road_0075)
  (road-next road_0072 road_0076)
  (road-next road_0072 road_0077)
  (connects road_0073 loc_0043 loc_0042)
  (road-open road_0073)
  (= (congestion-factor road_0073) 1.05)
  (= (travel-duration road_0073) 8.7044)
  (road-next road_0073 road_0066)
  (road-next road_0073 road_0067)
  (road-next road_0073 road_0068)
  (road-next road_0073 road_0069)
  (connects road_0074 loc_0044 loc_0040)
  (road-open road_0074)
  (= (congestion-factor road_0074) 1.35)
  (= (travel-duration road_0074) 6.9598)
  (road-next road_0074 road_0064)
  (connects road_0075 loc_0045 loc_0034)
  (road-open road_0075)
  (= (congestion-factor road_0075) 1.15)
  (= (travel-duration road_0075) 26.6544)
  (road-next road_0075 macro_0005)
  (road-next road_0075 road_0053)
  (connects road_0076 loc_0045 loc_0044)
  (road-open road_0076)
  (= (congestion-factor road_0076) 1.15)
  (= (travel-duration road_0076) 34.2654)
  (road-next road_0076 road_0074)
  (connects road_0077 loc_0045 loc_0043)
  (road-open road_0077)
  (= (congestion-factor road_0077) 1.1)
  (= (travel-duration road_0077) 5.131)
  (road-next road_0077 road_0070)
  (road-next road_0077 road_0071)
  (road-next road_0077 road_0072)
  (road-next road_0077 road_0073)
  (connects road_0078 loc_0046 loc_0053)
  (road-open road_0078)
  (= (congestion-factor road_0078) 1.0)
  (= (travel-duration road_0078) 22.5765)
  (road-next road_0078 macro_0010)
  (road-next road_0078 road_0090)
  (connects road_0079 loc_0046 loc_0005)
  (road-open road_0079)
  (= (congestion-factor road_0079) 1.0)
  (= (travel-duration road_0079) 4.547)
  (road-next road_0079 road_0008)
  (connects road_0080 loc_0047 loc_0049)
  (road-open road_0080)
  (= (congestion-factor road_0080) 1.03)
  (= (travel-duration road_0080) 5.2137)
  (road-next road_0080 road_0082)
  (road-next road_0080 road_0083)
  (connects road_0081 loc_0048 loc_0004)
  (road-open road_0081)
  (= (congestion-factor road_0081) 1.0)
  (= (travel-duration road_0081) 13.9002)
  (road-next road_0081 road_0006)
  (road-next road_0081 road_0007)
  (connects road_0082 loc_0049 loc_0087)
  (road-open road_0082)
  (= (congestion-factor road_0082) 1.0)
  (= (travel-duration road_0082) 8.0787)
  (road-next road_0082 road_0140)
  (connects road_0083 loc_0049 loc_0216)
  (road-open road_0083)
  (= (congestion-factor road_0083) 1.09)
  (= (travel-duration road_0083) 1.6298)
  (road-next road_0083 road_0376)
  (connects road_0084 loc_0050 loc_0029)
  (road-open road_0084)
  (= (congestion-factor road_0084) 1.0)
  (= (travel-duration road_0084) 18.8516)
  (road-next road_0084 road_0044)
  (road-next road_0084 road_0045)
  (connects road_0085 loc_0050 loc_0119)
  (road-open road_0085)
  (= (congestion-factor road_0085) 1.09)
  (= (travel-duration road_0085) 23.299)
  (road-next road_0085 road_0204)
  (road-next road_0085 road_0205)
  (connects road_0086 loc_0051 loc_0059)
  (road-open road_0086)
  (= (congestion-factor road_0086) 1.15)
  (= (travel-duration road_0086) 7.4344)
  (road-next road_0086 road_0100)
  (road-next road_0086 road_0101)
  (connects road_0087 loc_0051 loc_0052)
  (road-open road_0087)
  (= (congestion-factor road_0087) 1.09)
  (= (travel-duration road_0087) 3.8849)
  (road-next road_0087 macro_0009)
  (road-next road_0087 road_0088)
  (connects road_0088 loc_0052 loc_0051)
  (road-open road_0088)
  (= (congestion-factor road_0088) 1.0)
  (= (travel-duration road_0088) 9.7441)
  (road-next road_0088 road_0086)
  (road-next road_0088 road_0087)
  (connects road_0090 loc_0053 loc_0047)
  (road-open road_0090)
  (= (congestion-factor road_0090) 1.05)
  (= (travel-duration road_0090) 11.4249)
  (road-next road_0090 road_0080)
  (connects road_0094 loc_0055 loc_0116)
  (road-open road_0094)
  (= (congestion-factor road_0094) 1.06)
  (= (travel-duration road_0094) 10.4245)
  (road-next road_0094 road_0198)
  (road-next road_0094 road_0199)
  (road-next road_0094 road_0200)
  (connects road_0095 loc_0055 loc_0011)
  (road-open road_0095)
  (= (congestion-factor road_0095) 1.1)
  (= (travel-duration road_0095) 26.4037)
  (road-next road_0095 road_0015)
  (connects road_0096 loc_0056 loc_0064)
  (road-open road_0096)
  (= (congestion-factor road_0096) 1.29)
  (= (travel-duration road_0096) 5.6257)
  (road-next road_0096 road_0107)
  (connects road_0097 loc_0057 loc_0023)
  (road-open road_0097)
  (= (congestion-factor road_0097) 1.0)
  (= (travel-duration road_0097) 15.3114)
  (road-next road_0097 road_0032)
  (connects road_0099 loc_0058 loc_0051)
  (road-open road_0099)
  (= (congestion-factor road_0099) 1.09)
  (= (travel-duration road_0099) 32.3097)
  (road-next road_0099 road_0086)
  (road-next road_0099 road_0087)
  (connects road_0100 loc_0059 loc_0060)
  (road-open road_0100)
  (= (congestion-factor road_0100) 1.15)
  (= (travel-duration road_0100) 43.9175)
  (road-next road_0100 road_0102)
  (road-next road_0100 road_0103)
  (connects road_0101 loc_0059 loc_0061)
  (road-open road_0101)
  (= (congestion-factor road_0101) 1.0)
  (= (travel-duration road_0101) 13.9194)
  (road-next road_0101 road_0104)
  (connects road_0102 loc_0060 loc_0024)
  (road-open road_0102)
  (= (congestion-factor road_0102) 1.2)
  (= (travel-duration road_0102) 4.2323)
  (road-next road_0102 road_0033)
  (road-next road_0102 road_0034)
  (road-next road_0102 road_0035)
  (connects road_0103 loc_0060 loc_0147)
  (road-open road_0103)
  (= (congestion-factor road_0103) 1.14)
  (= (travel-duration road_0103) 11.9472)
  (road-next road_0103 road_0260)
  (road-next road_0103 road_0261)
  (road-next road_0103 road_0262)
  (connects road_0104 loc_0061 loc_0025)
  (road-open road_0104)
  (= (congestion-factor road_0104) 1.1)
  (= (travel-duration road_0104) 24.0949)
  (road-next road_0104 road_0036)
  (road-next road_0104 road_0037)
  (connects road_0105 loc_0062 loc_0051)
  (road-open road_0105)
  (= (congestion-factor road_0105) 1.06)
  (= (travel-duration road_0105) 23.5893)
  (road-next road_0105 road_0086)
  (road-next road_0105 road_0087)
  (connects road_0107 loc_0064 loc_0065)
  (road-open road_0107)
  (= (congestion-factor road_0107) 1.14)
  (= (travel-duration road_0107) 10.0265)
  (road-next road_0107 road_0108)
  (road-next road_0107 road_0109)
  (connects road_0108 loc_0065 loc_0164)
  (road-open road_0108)
  (= (congestion-factor road_0108) 1.11)
  (= (travel-duration road_0108) 14.3635)
  (road-next road_0108 road_0290)
  (road-next road_0108 road_0291)
  (connects road_0109 loc_0065 loc_0066)
  (road-open road_0109)
  (= (congestion-factor road_0109) 1.45)
  (= (travel-duration road_0109) 5.5856)
  (road-next road_0109 road_0110)
  (road-next road_0109 road_0111)
  (connects road_0110 loc_0066 loc_0270)
  (road-open road_0110)
  (= (congestion-factor road_0110) 1.15)
  (= (travel-duration road_0110) 7.1087)
  (road-next road_0110 road_0457)
  (road-next road_0110 road_0458)
  (connects road_0111 loc_0066 loc_0056)
  (road-open road_0111)
  (= (congestion-factor road_0111) 1.35)
  (= (travel-duration road_0111) 12.9427)
  (road-next road_0111 road_0096)
  (connects road_0112 loc_0067 loc_0068)
  (road-open road_0112)
  (= (congestion-factor road_0112) 1.03)
  (= (travel-duration road_0112) 6.5535)
  (road-next road_0112 road_0113)
  (road-next road_0112 road_0114)
  (connects road_0113 loc_0068 loc_0069)
  (road-open road_0113)
  (= (congestion-factor road_0113) 1.09)
  (= (travel-duration road_0113) 5.3565)
  (road-next road_0113 road_0115)
  (road-next road_0113 road_0116)
  (connects road_0114 loc_0068 loc_0183)
  (road-open road_0114)
  (= (congestion-factor road_0114) 1.11)
  (= (travel-duration road_0114) 7.8858)
  (road-next road_0114 road_0317)
  (road-next road_0114 road_0318)
  (connects road_0115 loc_0069 loc_0068)
  (road-open road_0115)
  (= (congestion-factor road_0115) 1.0)
  (= (travel-duration road_0115) 4.9142)
  (road-next road_0115 road_0113)
  (road-next road_0115 road_0114)
  (connects road_0116 loc_0069 loc_0185)
  (road-open road_0116)
  (= (congestion-factor road_0116) 1.1)
  (= (travel-duration road_0116) 11.0858)
  (connects road_0117 loc_0070 loc_0016)
  (road-open road_0117)
  (= (congestion-factor road_0117) 1.35)
  (= (travel-duration road_0117) 8.0695)
  (road-next road_0117 macro_0002)
  (road-next road_0117 macro_0003)
  (connects road_0118 loc_0071 loc_0047)
  (road-open road_0118)
  (= (congestion-factor road_0118) 1.0)
  (= (travel-duration road_0118) 48.3079)
  (road-next road_0118 road_0080)
  (connects road_0120 loc_0073 loc_0249)
  (road-open road_0120)
  (= (congestion-factor road_0120) 1.03)
  (= (travel-duration road_0120) 18.0762)
  (road-next road_0120 road_0425)
  (connects road_0121 loc_0074 loc_0180)
  (road-open road_0121)
  (= (congestion-factor road_0121) 1.14)
  (= (travel-duration road_0121) 7.898)
  (road-next road_0121 road_0315)
  (road-next road_0121 road_0316)
  (connects road_0122 loc_0075 loc_0228)
  (road-open road_0122)
  (= (congestion-factor road_0122) 1.06)
  (= (travel-duration road_0122) 4.6761)
  (road-next road_0122 road_0392)
  (connects road_0123 loc_0076 loc_0141)
  (road-open road_0123)
  (= (congestion-factor road_0123) 1.0)
  (= (travel-duration road_0123) 11.9141)
  (road-next road_0123 road_0250)
  (road-next road_0123 road_0251)
  (connects road_0124 loc_0077 loc_0247)
  (road-open road_0124)
  (= (congestion-factor road_0124) 1.23)
  (= (travel-duration road_0124) 5.274)
  (road-next road_0124 road_0422)
  (road-next road_0124 road_0423)
  (connects road_0125 loc_0078 loc_0160)
  (road-open road_0125)
  (= (congestion-factor road_0125) 1.15)
  (= (travel-duration road_0125) 20.6815)
  (road-next road_0125 road_0281)
  (road-next road_0125 road_0282)
  (connects road_0126 loc_0079 loc_0161)
  (road-open road_0126)
  (= (congestion-factor road_0126) 1.05)
  (= (travel-duration road_0126) 4.9948)
  (road-next road_0126 macro_0026)
  (road-next road_0126 road_0283)
  (road-next road_0126 road_0285)
  (connects road_0127 loc_0079 loc_0080)
  (road-open road_0127)
  (= (congestion-factor road_0127) 1.1)
  (= (travel-duration road_0127) 17.195)
  (road-next road_0127 road_0129)
  (connects road_0128 loc_0079 loc_0296)
  (road-open road_0128)
  (= (congestion-factor road_0128) 1.2)
  (= (travel-duration road_0128) 18.4875)
  (road-next road_0128 road_0498)
  (connects road_0129 loc_0080 loc_0079)
  (road-open road_0129)
  (= (congestion-factor road_0129) 1.05)
  (= (travel-duration road_0129) 16.4134)
  (road-next road_0129 road_0126)
  (road-next road_0129 road_0127)
  (road-next road_0129 road_0128)
  (connects road_0130 loc_0081 loc_0161)
  (road-open road_0130)
  (= (congestion-factor road_0130) 1.2)
  (= (travel-duration road_0130) 21.9839)
  (road-next road_0130 macro_0026)
  (road-next road_0130 road_0283)
  (road-next road_0130 road_0285)
  (connects road_0131 loc_0081 loc_0108)
  (road-open road_0131)
  (= (congestion-factor road_0131) 1.0)
  (= (travel-duration road_0131) 9.3448)
  (road-next road_0131 road_0184)
  (road-next road_0131 road_0185)
  (connects road_0132 loc_0081 loc_0126)
  (road-open road_0132)
  (= (congestion-factor road_0132) 1.15)
  (= (travel-duration road_0132) 19.1897)
  (road-next road_0132 road_0216)
  (road-next road_0132 road_0217)
  (connects road_0133 loc_0082 loc_0272)
  (road-open road_0133)
  (= (congestion-factor road_0133) 1.0)
  (= (travel-duration road_0133) 17.2399)
  (road-next road_0133 road_0460)
  (road-next road_0133 road_0461)
  (connects road_0134 loc_0083 loc_0254)
  (road-open road_0134)
  (= (congestion-factor road_0134) 1.05)
  (= (travel-duration road_0134) 8.5406)
  (road-next road_0134 road_0432)
  (road-next road_0134 road_0433)
  (connects road_0135 loc_0083 loc_0082)
  (road-open road_0135)
  (= (congestion-factor road_0135) 1.0)
  (= (travel-duration road_0135) 10.462)
  (road-next road_0135 road_0133)
  (connects road_0137 loc_0085 loc_0084)
  (road-open road_0137)
  (= (congestion-factor road_0137) 1.1)
  (= (travel-duration road_0137) 107.549)
  (road-next road_0137 macro_0015)
  (connects road_0138 loc_0086 loc_0143)
  (road-open road_0138)
  (= (congestion-factor road_0138) 1.25)
  (= (travel-duration road_0138) 2.9221)
  (road-next road_0138 road_0254)
  (connects road_0139 loc_0086 loc_0122)
  (road-open road_0139)
  (= (congestion-factor road_0139) 1.05)
  (= (travel-duration road_0139) 7.7909)
  (road-next road_0139 road_0209)
  (road-next road_0139 road_0210)
  (connects road_0140 loc_0087 loc_0070)
  (road-open road_0140)
  (= (congestion-factor road_0140) 1.1)
  (= (travel-duration road_0140) 26.9502)
  (road-next road_0140 road_0117)
  (connects road_0141 loc_0088 loc_0182)
  (road-open road_0141)
  (= (congestion-factor road_0141) 1.0)
  (= (travel-duration road_0141) 6.4144)
  (connects road_0142 loc_0088 loc_0278)
  (road-open road_0142)
  (= (congestion-factor road_0142) 1.0)
  (= (travel-duration road_0142) 18.7544)
  (connects road_0143 loc_0089 loc_0027)
  (road-open road_0143)
  (= (congestion-factor road_0143) 1.05)
  (= (travel-duration road_0143) 52.3639)
  (road-next road_0143 road_0041)
  (road-next road_0143 road_0042)
  (connects road_0144 loc_0089 loc_0090)
  (road-open road_0144)
  (= (congestion-factor road_0144) 1.1)
  (= (travel-duration road_0144) 15.8665)
  (road-next road_0144 road_0145)
  (connects road_0145 loc_0090 loc_0025)
  (road-open road_0145)
  (= (congestion-factor road_0145) 1.15)
  (= (travel-duration road_0145) 66.45)
  (road-next road_0145 road_0036)
  (road-next road_0145 road_0037)
  (connects road_0146 loc_0091 loc_0089)
  (road-open road_0146)
  (= (congestion-factor road_0146) 1.2)
  (= (travel-duration road_0146) 26.4665)
  (road-next road_0146 road_0143)
  (road-next road_0146 road_0144)
  (connects road_0147 loc_0091 loc_0257)
  (road-open road_0147)
  (= (congestion-factor road_0147) 1.1)
  (= (travel-duration road_0147) 25.1298)
  (road-next road_0147 road_0437)
  (road-next road_0147 road_0438)
  (connects road_0148 loc_0092 loc_0062)
  (road-open road_0148)
  (= (congestion-factor road_0148) 1.1)
  (= (travel-duration road_0148) 23.4837)
  (road-next road_0148 road_0105)
  (connects road_0149 loc_0093 loc_0282)
  (road-open road_0149)
  ;; [DYNAMIC EVENT - accident] An accident on road_0149, the narrow link between loc_0093 and loc_0282, closes a low‑degree connector that is part of the main route, causing a detour.
  (road-blocked road_0149)
  (= (congestion-factor road_0149) 1.0)
  (= (travel-duration road_0149) 3.9278)
  (road-next road_0149 road_0473)
  (connects road_0150 loc_0093 loc_0092)
  (road-open road_0150)
  ;; [DYNAMIC EVENT - location closure] loc_0093 incident road
  (road-blocked road_0150)
  (= (congestion-factor road_0150) 1.0)
  (= (travel-duration road_0150) 28.0814)
  (road-next road_0150 road_0148)
  (connects road_0151 loc_0094 loc_0012)
  (road-open road_0151)
  ;; [DYNAMIC EVENT - location closure] loc_0094 incident road
  (road-blocked road_0151)
  (= (congestion-factor road_0151) 1.0)
  (= (travel-duration road_0151) 16.8932)
  (road-next road_0151 macro_0001)
  (road-next road_0151 road_0016)
  (road-next road_0151 road_0017)
  (connects road_0152 loc_0094 loc_0267)
  (road-open road_0152)
  ;; [DYNAMIC EVENT - roadworks] Roadworks on roads 0152, 0153 and 0454 at intersection loc_0094 block a key junction near the corridor, forcing traffic to detour around the shared node.
  (road-blocked road_0152)
  (= (congestion-factor road_0152) 1.0)
  (= (travel-duration road_0152) 7.2351)
  (road-next road_0152 road_0453)
  (road-next road_0152 road_0454)
  (connects road_0153 loc_0094 loc_0093)
  (road-open road_0153)
  ;; [DYNAMIC EVENT - roadworks] Roadworks on roads 0152, 0153 and 0454 at intersection loc_0094 block a key junction near the corridor, forcing traffic to detour around the shared node.
  (road-blocked road_0153)
  (= (congestion-factor road_0153) 1.0)
  (= (travel-duration road_0153) 10.408)
  (road-next road_0153 road_0149)
  (road-next road_0153 road_0150)
  (connects road_0155 loc_0096 loc_0168)
  (road-open road_0155)
  (= (congestion-factor road_0155) 1.1)
  (= (travel-duration road_0155) 52.3351)
  (road-next road_0155 road_0297)
  (connects road_0157 loc_0097 loc_0096)
  (road-open road_0157)
  (= (congestion-factor road_0157) 1.15)
  (= (travel-duration road_0157) 37.2574)
  (road-next road_0157 macro_0017)
  (road-next road_0157 road_0155)
  (connects road_0158 loc_0097 loc_0166)
  (road-open road_0158)
  (= (congestion-factor road_0158) 1.14)
  (= (travel-duration road_0158) 22.8643)
  (road-next road_0158 road_0294)
  (connects road_0159 loc_0097 loc_0024)
  (road-open road_0159)
  (= (congestion-factor road_0159) 1.14)
  (= (travel-duration road_0159) 19.5796)
  (road-next road_0159 road_0033)
  (road-next road_0159 road_0034)
  (road-next road_0159 road_0035)
  (connects road_0161 loc_0099 loc_0226)
  (road-open road_0161)
  (= (congestion-factor road_0161) 1.15)
  (= (travel-duration road_0161) 4.4355)
  (road-next road_0161 road_0390)
  (connects road_0162 loc_0099 loc_0227)
  (road-open road_0162)
  (= (congestion-factor road_0162) 1.06)
  (= (travel-duration road_0162) 2.862)
  (road-next road_0162 road_0391)
  (connects road_0163 loc_0100 loc_0292)
  (road-open road_0163)
  (= (congestion-factor road_0163) 1.1)
  (= (travel-duration road_0163) 14.1034)
  (road-next road_0163 road_0489)
  (road-next road_0163 road_0490)
  (connects road_0164 loc_0100 loc_0101)
  (road-open road_0164)
  (= (congestion-factor road_0164) 1.2)
  (= (travel-duration road_0164) 9.1876)
  (road-next road_0164 road_0165)
  (road-next road_0164 road_0166)
  (road-next road_0164 road_0167)
  (connects road_0165 loc_0101 loc_0100)
  (road-open road_0165)
  (= (congestion-factor road_0165) 1.05)
  (= (travel-duration road_0165) 8.0391)
  (road-next road_0165 road_0163)
  (road-next road_0165 road_0164)
  (connects road_0166 loc_0101 loc_0204)
  (road-open road_0166)
  (= (congestion-factor road_0166) 1.25)
  (= (travel-duration road_0166) 2.0011)
  (road-next road_0166 road_0354)
  (road-next road_0166 road_0355)
  (road-next road_0166 road_0356)
  (connects road_0167 loc_0101 loc_0201)
  (road-open road_0167)
  (= (congestion-factor road_0167) 1.1)
  (= (travel-duration road_0167) 0.9808)
  (road-next road_0167 road_0347)
  (road-next road_0167 road_0348)
  (connects road_0168 loc_0102 loc_0193)
  (road-open road_0168)
  (= (congestion-factor road_0168) 1.1)
  (= (travel-duration road_0168) 1.1199)
  (road-next road_0168 road_0331)
  (road-next road_0168 road_0332)
  (connects road_0169 loc_0102 loc_0212)
  (road-open road_0169)
  (= (congestion-factor road_0169) 1.1)
  (= (travel-duration road_0169) 7.5943)
  (road-next road_0169 road_0371)
  (road-next road_0169 road_0372)
  (connects road_0171 loc_0103 loc_0243)
  (road-open road_0171)
  (= (congestion-factor road_0171) 1.15)
  (= (travel-duration road_0171) 11.4986)
  (road-next road_0171 road_0417)
  (connects road_0172 loc_0103 loc_0195)
  (road-open road_0172)
  (= (congestion-factor road_0172) 1.0)
  (= (travel-duration road_0172) 1.4941)
  (road-next road_0172 road_0335)
  (road-next road_0172 road_0336)
  (road-next road_0172 road_0337)
  (connects road_0173 loc_0104 loc_0103)
  (road-open road_0173)
  (= (congestion-factor road_0173) 1.0)
  (= (travel-duration road_0173) 13.5161)
  (road-next road_0173 road_0171)
  (road-next road_0173 road_0172)
  (connects road_0174 loc_0104 loc_0176)
  (road-open road_0174)
  (= (congestion-factor road_0174) 1.0)
  (= (travel-duration road_0174) 2.0857)
  (road-next road_0174 macro_0027)
  (road-next road_0174 road_0307)
  (connects road_0175 loc_0105 loc_0140)
  (road-open road_0175)
  (= (congestion-factor road_0175) 1.09)
  (= (travel-duration road_0175) 13.6548)
  (road-next road_0175 road_0247)
  (road-next road_0175 road_0248)
  (road-next road_0175 road_0249)
  (connects road_0176 loc_0105 loc_0106)
  (road-open road_0176)
  (= (congestion-factor road_0176) 1.03)
  (= (travel-duration road_0176) 2.2397)
  (road-next road_0176 road_0178)
  (road-next road_0176 road_0179)
  (road-next road_0176 road_0180)
  (connects road_0177 loc_0105 loc_0014)
  (road-open road_0177)
  (= (congestion-factor road_0177) 1.04)
  (= (travel-duration road_0177) 18.4735)
  (road-next road_0177 road_0022)
  (road-next road_0177 road_0023)
  (connects road_0178 loc_0106 loc_0136)
  (road-open road_0178)
  (= (congestion-factor road_0178) 1.0)
  (= (travel-duration road_0178) 2.8801)
  (road-next road_0178 road_0238)
  (road-next road_0178 road_0239)
  (road-next road_0178 road_0240)
  (road-next road_0178 road_0241)
  (connects road_0179 loc_0106 loc_0105)
  (road-open road_0179)
  (= (congestion-factor road_0179) 1.03)
  (= (travel-duration road_0179) 2.2397)
  (road-next road_0179 road_0175)
  (road-next road_0179 road_0176)
  (road-next road_0179 road_0177)
  (connects road_0180 loc_0106 loc_0137)
  (road-open road_0180)
  (= (congestion-factor road_0180) 1.04)
  (= (travel-duration road_0180) 9.616)
  (road-next road_0180 road_0242)
  (connects road_0181 loc_0107 loc_0110)
  (road-open road_0181)
  (= (congestion-factor road_0181) 1.25)
  (= (travel-duration road_0181) 9.7999)
  (road-next road_0181 road_0188)
  (connects road_0182 loc_0107 loc_0029)
  (road-open road_0182)
  (= (congestion-factor road_0182) 1.03)
  (= (travel-duration road_0182) 44.237)
  (road-next road_0182 road_0044)
  (road-next road_0182 road_0045)
  (connects road_0183 loc_0107 loc_0109)
  (road-open road_0183)
  (= (congestion-factor road_0183) 1.0)
  (= (travel-duration road_0183) 12.6887)
  (road-next road_0183 road_0186)
  (road-next road_0183 road_0187)
  (connects road_0184 loc_0108 loc_0238)
  (road-open road_0184)
  (= (congestion-factor road_0184) 1.05)
  (= (travel-duration road_0184) 2.8628)
  (road-next road_0184 road_0407)
  (road-next road_0184 road_0408)
  (connects road_0185 loc_0108 loc_0081)
  (road-open road_0185)
  (= (congestion-factor road_0185) 1.3)
  (= (travel-duration road_0185) 12.1482)
  (road-next road_0185 road_0130)
  (road-next road_0185 road_0131)
  (road-next road_0185 road_0132)
  (connects road_0186 loc_0109 loc_0161)
  (road-open road_0186)
  (= (congestion-factor road_0186) 1.03)
  (= (travel-duration road_0186) 13.5187)
  (road-next road_0186 macro_0026)
  (road-next road_0186 road_0283)
  (road-next road_0186 road_0285)
  (connects road_0187 loc_0109 loc_0107)
  (road-open road_0187)
  (= (congestion-factor road_0187) 1.17)
  (= (travel-duration road_0187) 7.5215)
  (road-next road_0187 road_0181)
  (road-next road_0187 road_0182)
  (road-next road_0187 road_0183)
  (connects road_0188 loc_0110 loc_0217)
  (road-open road_0188)
  (= (congestion-factor road_0188) 1.35)
  (= (travel-duration road_0188) 4.2543)
  (road-next road_0188 road_0377)
  (connects road_0189 loc_0111 loc_0174)
  (road-open road_0189)
  (= (congestion-factor road_0189) 1.05)
  (= (travel-duration road_0189) 0.8959)
  (road-next road_0189 road_0304)
  (road-next road_0189 road_0305)
  (connects road_0190 loc_0111 loc_0110)
  (road-open road_0190)
  (= (congestion-factor road_0190) 1.1)
  (= (travel-duration road_0190) 7.8042)
  (road-next road_0190 road_0188)
  (connects road_0191 loc_0112 loc_0283)
  (road-open road_0191)
  (= (congestion-factor road_0191) 1.05)
  (= (travel-duration road_0191) 0.344)
  (road-next road_0191 road_0474)
  (road-next road_0191 road_0475)
  (road-next road_0191 road_0476)
  (connects road_0192 loc_0113 loc_0115)
  (road-open road_0192)
  (= (congestion-factor road_0192) 1.15)
  (= (travel-duration road_0192) 7.7697)
  (road-next road_0192 road_0197)
  (connects road_0193 loc_0113 loc_0114)
  (road-open road_0193)
  (= (congestion-factor road_0193) 1.1)
  (= (travel-duration road_0193) 10.9063)
  (road-next road_0193 road_0195)
  (road-next road_0193 road_0196)
  (connects road_0194 loc_0113 loc_0234)
  (road-open road_0194)
  (= (congestion-factor road_0194) 1.0)
  (= (travel-duration road_0194) 2.1997)
  (road-next road_0194 road_0401)
  (connects road_0195 loc_0114 loc_0008)
  (road-open road_0195)
  (= (congestion-factor road_0195) 1.25)
  (= (travel-duration road_0195) 5.5397)
  (road-next road_0195 macro_0000)
  (connects road_0196 loc_0114 loc_0113)
  (road-open road_0196)
  (= (congestion-factor road_0196) 1.05)
  (= (travel-duration road_0196) 10.4105)
  (road-next road_0196 road_0192)
  (road-next road_0196 road_0193)
  (road-next road_0196 road_0194)
  (connects road_0197 loc_0115 loc_0011)
  (road-open road_0197)
  (= (congestion-factor road_0197) 1.45)
  (= (travel-duration road_0197) 18.229)
  (road-next road_0197 road_0015)
  (connects road_0198 loc_0116 loc_0215)
  (road-open road_0198)
  (= (congestion-factor road_0198) 1.1)
  (= (travel-duration road_0198) 37.8308)
  (road-next road_0198 road_0375)
  (connects road_0199 loc_0116 loc_0268)
  (road-open road_0199)
  (= (congestion-factor road_0199) 1.03)
  (= (travel-duration road_0199) 11.185)
  (road-next road_0199 road_0455)
  (connects road_0200 loc_0116 loc_0055)
  (road-open road_0200)
  (= (congestion-factor road_0200) 1.0)
  (= (travel-duration road_0200) 9.8344)
  (road-next road_0200 road_0094)
  (road-next road_0200 road_0095)
  (connects road_0201 loc_0117 loc_0291)
  (road-open road_0201)
  (= (congestion-factor road_0201) 1.11)
  (= (travel-duration road_0201) 0.1732)
  (road-next road_0201 road_0487)
  (road-next road_0201 road_0488)
  (connects road_0202 loc_0117 loc_0290)
  (road-open road_0202)
  (= (congestion-factor road_0202) 1.09)
  (= (travel-duration road_0202) 0.3309)
  (road-next road_0202 road_0485)
  (road-next road_0202 road_0486)
  (connects road_0203 loc_0118 loc_0210)
  (road-open road_0203)
  (= (congestion-factor road_0203) 1.14)
  (= (travel-duration road_0203) 15.6099)
  (road-next road_0203 road_0366)
  (road-next road_0203 road_0367)
  (connects road_0204 loc_0119 loc_0001)
  (road-open road_0204)
  (= (congestion-factor road_0204) 1.09)
  (= (travel-duration road_0204) 9.4795)
  (road-next road_0204 road_0003)
  (road-next road_0204 road_0004)
  (connects road_0205 loc_0119 loc_0269)
  (road-open road_0205)
  (= (congestion-factor road_0205) 1.05)
  (= (travel-duration road_0205) 4.0725)
  (road-next road_0205 road_0456)
  (connects road_0206 loc_0120 loc_0086)
  (road-open road_0206)
  (= (congestion-factor road_0206) 1.3)
  (= (travel-duration road_0206) 6.6895)
  (road-next road_0206 road_0138)
  (road-next road_0206 road_0139)
  (connects road_0207 loc_0121 loc_0120)
  (road-open road_0207)
  (= (congestion-factor road_0207) 1.15)
  (= (travel-duration road_0207) 11.137)
  (road-next road_0207 road_0206)
  (connects road_0208 loc_0121 loc_0123)
  (road-open road_0208)
  (= (congestion-factor road_0208) 1.0)
  (= (travel-duration road_0208) 14.3598)
  (road-next road_0208 road_0211)
  (road-next road_0208 road_0212)
  (connects road_0209 loc_0122 loc_0121)
  (road-open road_0209)
  (= (congestion-factor road_0209) 1.0)
  (= (travel-duration road_0209) 4.9454)
  (road-next road_0209 road_0207)
  (road-next road_0209 road_0208)
  (connects road_0210 loc_0122 loc_0087)
  (road-open road_0210)
  (= (congestion-factor road_0210) 1.05)
  (= (travel-duration road_0210) 21.7069)
  (road-next road_0210 road_0140)
  (connects road_0211 loc_0123 loc_0001)
  (road-open road_0211)
  (= (congestion-factor road_0211) 1.09)
  (= (travel-duration road_0211) 21.2362)
  (road-next road_0211 road_0003)
  (road-next road_0211 road_0004)
  (connects road_0212 loc_0123 loc_0121)
  (road-open road_0212)
  (= (congestion-factor road_0212) 1.15)
  (= (travel-duration road_0212) 16.5137)
  (road-next road_0212 road_0207)
  (road-next road_0212 road_0208)
  (connects road_0213 loc_0124 loc_0149)
  (road-open road_0213)
  (= (congestion-factor road_0213) 1.1)
  (= (travel-duration road_0213) 1.4059)
  (road-next road_0213 road_0265)
  (road-next road_0213 road_0266)
  (connects road_0215 loc_0125 loc_0171)
  (road-open road_0215)
  (= (congestion-factor road_0215) 1.05)
  (= (travel-duration road_0215) 4.6874)
  (road-next road_0215 road_0300)
  (connects road_0216 loc_0126 loc_0124)
  (road-open road_0216)
  (= (congestion-factor road_0216) 2.5)  ;; [DYNAMIC EVENT - slowdown] Light congestion on road_0216 reduces speed by a factor of 2.5, affecting the segment between loc_0126 and loc_0124, which lies on the primary path.
  (= (travel-duration road_0216) 24.2373)  ;; [DYNAMIC EVENT - slowdown] Light congestion on road_0216 reduces speed by a factor of 2.5, affecting the segment between loc_0126 and loc_0124, which lies on the primary path.
  (road-next road_0216 road_0213)
  (connects road_0217 loc_0126 loc_0058)
  (road-open road_0217)
  (= (congestion-factor road_0217) 1.1)
  (= (travel-duration road_0217) 12.5167)
  (road-next road_0217 road_0099)
  (connects road_0218 loc_0127 loc_0055)
  (road-open road_0218)
  (= (congestion-factor road_0218) 1.14)
  (= (travel-duration road_0218) 7.9361)
  (road-next road_0218 road_0094)
  (road-next road_0218 road_0095)
  (connects road_0219 loc_0128 loc_0288)
  (road-open road_0219)
  (= (congestion-factor road_0219) 1.05)
  (= (travel-duration road_0219) 3.3568)
  (road-next road_0219 road_0482)
  (road-next road_0219 road_0483)
  (connects road_0220 loc_0129 loc_0130)
  (road-open road_0220)
  (= (congestion-factor road_0220) 1.0)
  (= (travel-duration road_0220) 13.4981)
  (road-next road_0220 road_0221)
  (road-next road_0220 road_0222)
  (road-next road_0220 road_0223)
  (connects road_0221 loc_0130 loc_0134)
  (road-open road_0221)
  (= (congestion-factor road_0221) 1.1)
  (= (travel-duration road_0221) 56.6263)
  (road-next road_0221 road_0232)
  (road-next road_0221 road_0233)
  (road-next road_0221 road_0234)
  (connects road_0222 loc_0130 loc_0129)
  (road-open road_0222)
  (= (congestion-factor road_0222) 1.0)
  (= (travel-duration road_0222) 13.4981)
  (road-next road_0222 road_0220)
  (connects road_0223 loc_0130 loc_0133)
  (road-open road_0223)
  (= (congestion-factor road_0223) 1.1)
  (= (travel-duration road_0223) 13.223)
  (road-next road_0223 macro_0023)
  (road-next road_0223 macro_0024)
  (connects road_0226 loc_0131 loc_0132)
  (road-open road_0226)
  (= (congestion-factor road_0226) 1.0)
  (= (travel-duration road_0226) 18.3451)
  (road-next road_0226 road_0227)
  (road-next road_0226 road_0228)
  (connects road_0227 loc_0132 loc_0131)
  (road-open road_0227)
  (= (congestion-factor road_0227) 1.15)
  (= (travel-duration road_0227) 21.0969)
  (road-next road_0227 macro_0021)
  (road-next road_0227 macro_0022)
  (road-next road_0227 road_0226)
  (connects road_0228 loc_0132 loc_0236)
  (road-open road_0228)
  (= (congestion-factor road_0228) 1.06)
  (= (travel-duration road_0228) 9.8536)
  (road-next road_0228 road_0403)
  (road-next road_0228 road_0404)
  (connects road_0232 loc_0134 loc_0135)
  (road-open road_0232)
  (= (congestion-factor road_0232) 1.05)
  (= (travel-duration road_0232) 18.1636)
  (road-next road_0232 road_0235)
  (road-next road_0232 road_0236)
  (road-next road_0232 road_0237)
  (connects road_0233 loc_0134 loc_0214)
  (road-open road_0233)
  (= (congestion-factor road_0233) 1.05)
  (= (travel-duration road_0233) 3.2131)
  (road-next road_0233 road_0374)
  (connects road_0234 loc_0134 loc_0130)
  (road-open road_0234)
  (= (congestion-factor road_0234) 1.0)
  (= (travel-duration road_0234) 51.4785)
  (road-next road_0234 road_0221)
  (road-next road_0234 road_0222)
  (road-next road_0234 road_0223)
  (connects road_0235 loc_0135 loc_0013)
  (road-open road_0235)
  (= (congestion-factor road_0235) 1.1)
  (= (travel-duration road_0235) 23.2884)
  (road-next road_0235 road_0019)
  (road-next road_0235 road_0020)
  (road-next road_0235 road_0021)
  (connects road_0236 loc_0135 loc_0134)
  (road-open road_0236)
  (= (congestion-factor road_0236) 1.0)
  (= (travel-duration road_0236) 17.2987)
  (road-next road_0236 road_0232)
  (road-next road_0236 road_0233)
  (road-next road_0236 road_0234)
  (connects road_0237 loc_0135 loc_0213)
  (road-open road_0237)
  (= (congestion-factor road_0237) 1.0)
  (= (travel-duration road_0237) 5.2346)
  (road-next road_0237 road_0373)
  (connects road_0238 loc_0136 loc_0261)
  (road-open road_0238)
  (= (congestion-factor road_0238) 1.0)
  (= (travel-duration road_0238) 20.6252)
  (road-next road_0238 road_0443)
  (road-next road_0238 road_0444)
  (connects road_0239 loc_0136 loc_0138)
  (road-open road_0239)
  (= (congestion-factor road_0239) 1.05)
  (= (travel-duration road_0239) 14.7741)
  (road-next road_0239 road_0243)
  (connects road_0240 loc_0136 loc_0106)
  (road-open road_0240)
  (= (congestion-factor road_0240) 1.03)
  (= (travel-duration road_0240) 2.9665)
  (road-next road_0240 road_0178)
  (road-next road_0240 road_0179)
  (road-next road_0240 road_0180)
  (connects road_0241 loc_0136 loc_0181)
  (road-open road_0241)
  (= (congestion-factor road_0241) 1.0)
  (= (travel-duration road_0241) 7.5627)
  (connects road_0242 loc_0137 loc_0139)
  (road-open road_0242)
  (= (congestion-factor road_0242) 1.08)
  (= (travel-duration road_0242) 4.8055)
  (road-next road_0242 road_0244)
  (road-next road_0242 road_0245)
  (road-next road_0242 road_0246)
  (connects road_0243 loc_0138 loc_0137)
  (road-open road_0243)
  (= (congestion-factor road_0243) 1.05)
  (= (travel-duration road_0243) 3.2219)
  (road-next road_0243 road_0242)
  (connects road_0244 loc_0139 loc_0138)
  (road-open road_0244)
  (= (congestion-factor road_0244) 1.05)
  (= (travel-duration road_0244) 11.5496)
  (road-next road_0244 road_0243)
  (connects road_0245 loc_0139 loc_0297)
  (road-open road_0245)
  (= (congestion-factor road_0245) 1.02)
  (= (travel-duration road_0245) 1.1985)
  (road-next road_0245 road_0499)
  (road-next road_0245 road_0500)
  (connects road_0246 loc_0139 loc_0277)
  (road-open road_0246)
  (= (congestion-factor road_0246) 1.04)
  (= (travel-duration road_0246) 19.4701)
  (road-next road_0246 road_0468)
  (connects road_0247 loc_0140 loc_0274)
  (road-open road_0247)
  (= (congestion-factor road_0247) 1.05)
  (= (travel-duration road_0247) 12.7378)
  (road-next road_0247 road_0464)
  (connects road_0248 loc_0140 loc_0105)
  (road-open road_0248)
  (= (congestion-factor road_0248) 1.03)
  (= (travel-duration road_0248) 12.9031)
  (road-next road_0248 road_0175)
  (road-next road_0248 road_0176)
  (road-next road_0248 road_0177)
  (connects road_0249 loc_0140 loc_0288)
  (road-open road_0249)
  (= (congestion-factor road_0249) 1.09)
  (= (travel-duration road_0249) 3.0504)
  (road-next road_0249 road_0482)
  (road-next road_0249 road_0483)
  (connects road_0250 loc_0141 loc_0076)
  (road-open road_0250)
  (= (congestion-factor road_0250) 1.0)
  (= (travel-duration road_0250) 11.9141)
  (road-next road_0250 road_0123)
  (connects road_0251 loc_0141 loc_0192)
  (road-open road_0251)
  (= (congestion-factor road_0251) 1.05)
  (= (travel-duration road_0251) 11.8445)
  (road-next road_0251 road_0328)
  (road-next road_0251 road_0329)
  (road-next road_0251 road_0330)
  (connects road_0252 loc_0142 loc_0184)
  (road-open road_0252)
  (= (congestion-factor road_0252) 1.11)
  (= (travel-duration road_0252) 2.4683)
  (road-next road_0252 macro_0028)
  (connects road_0253 loc_0142 loc_0164)
  (road-open road_0253)
  (= (congestion-factor road_0253) 1.11)
  (= (travel-duration road_0253) 14.0292)
  (road-next road_0253 road_0290)
  (road-next road_0253 road_0291)
  (connects road_0254 loc_0143 loc_0144)
  (road-open road_0254)
  (= (congestion-factor road_0254) 1.25)
  (= (travel-duration road_0254) 24.9715)
  (road-next road_0254 road_0255)
  (connects road_0255 loc_0144 loc_0070)
  (road-open road_0255)
  (= (congestion-factor road_0255) 1.3)
  (= (travel-duration road_0255) 9.8315)
  (road-next road_0255 road_0117)
  (connects road_0256 loc_0145 loc_0153)
  (road-open road_0256)
  (= (congestion-factor road_0256) 1.0)
  (= (travel-duration road_0256) 8.2839)
  (road-next road_0256 road_0270)
  (connects road_0257 loc_0146 loc_0218)
  (road-open road_0257)
  (= (congestion-factor road_0257) 1.0)
  (= (travel-duration road_0257) 8.6895)
  (road-next road_0257 macro_0030)
  (connects road_0258 loc_0146 loc_0152)
  (road-open road_0258)
  (= (congestion-factor road_0258) 1.0)
  (= (travel-duration road_0258) 7.9167)
  (road-next road_0258 road_0269)
  (connects road_0259 loc_0146 loc_0030)
  (road-open road_0259)
  (= (congestion-factor road_0259) 1.0)
  (= (travel-duration road_0259) 8.3463)
  (road-next road_0259 road_0046)
  (road-next road_0259 road_0047)
  (connects road_0260 loc_0147 loc_0148)
  (road-open road_0260)
  (= (congestion-factor road_0260) 1.15)
  (= (travel-duration road_0260) 25.4137)
  (road-next road_0260 road_0263)
  (road-next road_0260 road_0264)
  (connects road_0261 loc_0147 loc_0060)
  (road-open road_0261)
  (= (congestion-factor road_0261) 1.11)
  (= (travel-duration road_0261) 11.6328)
  (road-next road_0261 road_0102)
  (road-next road_0261 road_0103)
  (connects road_0262 loc_0147 loc_0025)
  (road-open road_0262)
  (= (congestion-factor road_0262) 1.14)
  (= (travel-duration road_0262) 6.8581)
  (road-next road_0262 road_0036)
  (road-next road_0262 road_0037)
  (connects road_0263 loc_0148 loc_0169)
  (road-open road_0263)
  (= (congestion-factor road_0263) 1.1)
  (= (travel-duration road_0263) 6.7481)
  (road-next road_0263 road_0298)
  (connects road_0264 loc_0148 loc_0061)
  (road-open road_0264)
  (= (congestion-factor road_0264) 1.05)
  (= (travel-duration road_0264) 3.416)
  (road-next road_0264 road_0104)
  (connects road_0265 loc_0149 loc_0172)
  (road-open road_0265)
  (= (congestion-factor road_0265) 1.05)
  (= (travel-duration road_0265) 6.0041)
  (road-next road_0265 road_0301)
  (connects road_0266 loc_0149 loc_0165)
  (road-open road_0266)
  (= (congestion-factor road_0266) 1.1)
  (= (travel-duration road_0266) 12.6527)
  (road-next road_0266 road_0292)
  (road-next road_0266 road_0293)
  (connects road_0269 loc_0152 loc_0146)
  (road-open road_0269)
  (= (congestion-factor road_0269) 1.0)
  (= (travel-duration road_0269) 7.9167)
  (road-next road_0269 road_0257)
  (road-next road_0269 road_0258)
  (road-next road_0269 road_0259)
  (connects road_0270 loc_0153 loc_0031)
  (road-open road_0270)
  (= (congestion-factor road_0270) 1.15)
  (= (travel-duration road_0270) 2.4344)
  (road-next road_0270 road_0048)
  (road-next road_0270 road_0049)
  (connects road_0272 loc_0154 loc_0155)
  (road-open road_0272)
  (= (congestion-factor road_0272) 1.0)
  (= (travel-duration road_0272) 11.6789)
  (road-next road_0272 road_0273)
  (road-next road_0272 road_0274)
  (road-next road_0272 road_0275)
  (connects road_0273 loc_0155 loc_0162)
  (road-open road_0273)
  (= (congestion-factor road_0273) 1.0)
  (= (travel-duration road_0273) 1.5349)
  (road-next road_0273 road_0286)
  (connects road_0274 loc_0155 loc_0157)
  (road-open road_0274)
  (= (congestion-factor road_0274) 1.05)
  (= (travel-duration road_0274) 8.8922)
  (road-next road_0274 road_0277)
  (connects road_0275 loc_0155 loc_0156)
  (road-open road_0275)
  (= (congestion-factor road_0275) 1.0)
  (= (travel-duration road_0275) 12.2885)
  (road-next road_0275 road_0276)
  (connects road_0276 loc_0156 loc_0153)
  (road-open road_0276)
  (= (congestion-factor road_0276) 1.05)
  (= (travel-duration road_0276) 9.3445)
  (road-next road_0276 road_0270)
  (connects road_0277 loc_0157 loc_0295)
  (road-open road_0277)
  (= (congestion-factor road_0277) 1.15)
  (= (travel-duration road_0277) 15.9438)
  (road-next road_0277 road_0496)
  (road-next road_0277 road_0497)
  (connects road_0278 loc_0158 loc_0263)
  (road-open road_0278)
  (= (congestion-factor road_0278) 1.06)
  (= (travel-duration road_0278) 5.2332)
  (road-next road_0278 road_0445)
  (road-next road_0278 road_0446)
  (connects road_0279 loc_0158 loc_0163)
  (road-open road_0279)
  (= (congestion-factor road_0279) 1.14)
  (= (travel-duration road_0279) 0.9946)
  (road-next road_0279 road_0287)
  (road-next road_0279 road_0288)
  (road-next road_0279 road_0289)
  (connects road_0280 loc_0159 loc_0117)
  (road-open road_0280)
  (= (congestion-factor road_0280) 1.17)
  (= (travel-duration road_0280) 9.9899)
  (road-next road_0280 road_0201)
  (road-next road_0280 road_0202)
  (connects road_0281 loc_0160 loc_0158)
  (road-open road_0281)
  (= (congestion-factor road_0281) 1.15)
  (= (travel-duration road_0281) 16.5455)
  (road-next road_0281 road_0278)
  (road-next road_0281 road_0279)
  (connects road_0282 loc_0160 loc_0159)
  (road-open road_0282)
  (= (congestion-factor road_0282) 1.15)
  (= (travel-duration road_0282) 6.8906)
  (road-next road_0282 road_0280)
  (connects road_0283 loc_0161 loc_0079)
  (road-open road_0283)
  (= (congestion-factor road_0283) 1.3)
  (= (travel-duration road_0283) 6.1841)
  (road-next road_0283 road_0126)
  (road-next road_0283 road_0127)
  (road-next road_0283 road_0128)
  (connects road_0285 loc_0161 loc_0109)
  (road-open road_0285)
  (= (congestion-factor road_0285) 1.14)
  (= (travel-duration road_0285) 21.2224)
  (road-next road_0285 road_0186)
  (road-next road_0285 road_0187)
  (connects road_0286 loc_0162 loc_0155)
  (road-open road_0286)
  (= (congestion-factor road_0286) 1.0)
  (= (travel-duration road_0286) 1.5349)
  (road-next road_0286 road_0273)
  (road-next road_0286 road_0274)
  (road-next road_0286 road_0275)
  (connects road_0287 loc_0163 loc_0012)
  (road-open road_0287)
  (= (congestion-factor road_0287) 1.14)
  (= (travel-duration road_0287) 17.1124)
  (road-next road_0287 macro_0001)
  (road-next road_0287 road_0016)
  (road-next road_0287 road_0017)
  (connects road_0288 loc_0163 loc_0158)
  (road-open road_0288)
  (= (congestion-factor road_0288) 1.0)
  (= (travel-duration road_0288) 0.8724)
  (road-next road_0288 road_0278)
  (road-next road_0288 road_0279)
  (connects road_0289 loc_0163 loc_0118)
  (road-open road_0289)
  (= (congestion-factor road_0289) 1.11)
  (= (travel-duration road_0289) 25.0946)
  (road-next road_0289 road_0203)
  (connects road_0290 loc_0164 loc_0142)
  (road-open road_0290)
  (= (congestion-factor road_0290) 1.11)
  (= (travel-duration road_0290) 14.0292)
  (road-next road_0290 road_0252)
  (road-next road_0290 road_0253)
  (connects road_0291 loc_0164 loc_0065)
  (road-open road_0291)
  (= (congestion-factor road_0291) 1.17)
  (= (travel-duration road_0291) 15.1399)
  (road-next road_0291 road_0108)
  (road-next road_0291 road_0109)
  (connects road_0292 loc_0165 loc_0125)
  (road-open road_0292)
  (= (congestion-factor road_0292) 1.1)
  (= (travel-duration road_0292) 8.3005)
  (road-next road_0292 macro_0020)
  (road-next road_0292 road_0215)
  (connects road_0293 loc_0165 loc_0170)
  (road-open road_0293)
  (= (congestion-factor road_0293) 1.0)
  (= (travel-duration road_0293) 21.1532)
  (road-next road_0293 road_0299)
  (goal-road road_0293)
  (connects road_0294 loc_0166 loc_0097)
  (road-open road_0294)
  (= (congestion-factor road_0294) 1.2)
  (= (travel-duration road_0294) 24.0677)
  (road-next road_0294 road_0157)
  (road-next road_0294 road_0158)
  (road-next road_0294 road_0159)
  (connects road_0295 loc_0167 loc_0063)
  (road-open road_0295)
  (= (congestion-factor road_0295) 1.05)
  (= (travel-duration road_0295) 6.2952)
  (road-next road_0295 macro_0014)
  (connects road_0297 loc_0168 loc_0096)
  (road-open road_0297)
  (= (congestion-factor road_0297) 1.0)
  (= (travel-duration road_0297) 47.5774)
  (road-next road_0297 macro_0017)
  (road-next road_0297 road_0155)
  (connects road_0298 loc_0169 loc_0148)
  (road-open road_0298)
  (= (congestion-factor road_0298) 1.0)
  (= (travel-duration road_0298) 6.1346)
  (road-next road_0298 road_0263)
  (road-next road_0298 road_0264)
  (connects road_0299 loc_0170 loc_0124)
  (road-open road_0299)
  (= (congestion-factor road_0299) 1.0)
  (= (travel-duration road_0299) 12.9149)
  (road-next road_0299 road_0213)
  (connects road_0300 loc_0171 loc_0125)
  (road-open road_0300)
  (= (congestion-factor road_0300) 1.0)
  (= (travel-duration road_0300) 4.4642)
  (road-next road_0300 macro_0020)
  (road-next road_0300 road_0215)
  (connects road_0301 loc_0172 loc_0108)
  (road-open road_0301)
  (= (congestion-factor road_0301) 1.25)
  (= (travel-duration road_0301) 14.4921)
  (road-next road_0301 road_0184)
  (road-next road_0301 road_0185)
  (connects road_0302 loc_0173 loc_0283)
  (road-open road_0302)
  (= (congestion-factor road_0302) 1.15)
  (= (travel-duration road_0302) 3.5771)
  (road-next road_0302 road_0474)
  (road-next road_0302 road_0475)
  (road-next road_0302 road_0476)
  (connects road_0303 loc_0173 loc_0217)
  (road-open road_0303)
  (= (congestion-factor road_0303) 1.0)
  (= (travel-duration road_0303) 5.5286)
  (road-next road_0303 road_0377)
  (connects road_0304 loc_0174 loc_0109)
  (road-open road_0304)
  (= (congestion-factor road_0304) 1.05)
  (= (travel-duration road_0304) 13.8468)
  (road-next road_0304 road_0186)
  (road-next road_0304 road_0187)
  (connects road_0305 loc_0174 loc_0294)
  (road-open road_0305)
  (= (congestion-factor road_0305) 1.05)
  (= (travel-duration road_0305) 6.3897)
  (road-next road_0305 road_0493)
  (road-next road_0305 road_0494)
  (road-next road_0305 road_0495)
  (connects road_0306 loc_0175 loc_0294)
  (road-open road_0306)
  (= (congestion-factor road_0306) 1.05)
  (= (travel-duration road_0306) 5.0944)
  (road-next road_0306 road_0493)
  (road-next road_0306 road_0494)
  (road-next road_0306 road_0495)
  (connects road_0307 loc_0176 loc_0018)
  (road-open road_0307)
  (= (congestion-factor road_0307) 1.0)
  (= (travel-duration road_0307) 3.9434)
  (connects road_0309 loc_0177 loc_0289)
  (road-open road_0309)
  (= (congestion-factor road_0309) 1.05)
  (= (travel-duration road_0309) 7.6208)
  (road-next road_0309 road_0484)
  (connects road_0310 loc_0177 loc_0040)
  (road-open road_0310)
  (= (congestion-factor road_0310) 1.1)
  (= (travel-duration road_0310) 25.349)
  (road-next road_0310 road_0064)
  (connects road_0311 loc_0178 loc_0289)
  (road-open road_0311)
  (= (congestion-factor road_0311) 1.06)
  (= (travel-duration road_0311) 6.6795)
  (road-next road_0311 road_0484)
  (connects road_0312 loc_0178 loc_0042)
  (road-open road_0312)
  (= (congestion-factor road_0312) 1.09)
  (= (travel-duration road_0312) 4.6554)
  (road-next road_0312 road_0066)
  (road-next road_0312 road_0067)
  (road-next road_0312 road_0068)
  (road-next road_0312 road_0069)
  (connects road_0313 loc_0179 loc_0077)
  (road-open road_0313)
  (= (congestion-factor road_0313) 1.15)
  (= (travel-duration road_0313) 2.6525)
  (road-next road_0313 road_0124)
  (connects road_0314 loc_0179 loc_0042)
  (road-open road_0314)
  (= (congestion-factor road_0314) 1.06)
  (= (travel-duration road_0314) 6.7673)
  (road-next road_0314 road_0066)
  (road-next road_0314 road_0067)
  (road-next road_0314 road_0068)
  (road-next road_0314 road_0069)
  (connects road_0315 loc_0180 loc_0202)
  (road-open road_0315)
  (= (congestion-factor road_0315) 1.1)
  (= (travel-duration road_0315) 17.9738)
  (road-next road_0315 road_0349)
  (road-next road_0315 road_0350)
  (connects road_0316 loc_0180 loc_0189)
  (road-open road_0316)
  (= (congestion-factor road_0316) 1.11)
  (= (travel-duration road_0316) 44.8116)
  (road-next road_0316 road_0325)
  (connects road_0317 loc_0183 loc_0142)
  (road-open road_0317)
  (= (congestion-factor road_0317) 1.06)
  (= (travel-duration road_0317) 2.221)
  (road-next road_0317 road_0252)
  (road-next road_0317 road_0253)
  (connects road_0318 loc_0183 loc_0184)
  (road-open road_0318)
  (= (congestion-factor road_0318) 1.03)
  (= (travel-duration road_0318) 1.2398)
  (road-next road_0318 macro_0028)
  (connects road_0321 loc_0187 loc_0281)
  (road-open road_0321)
  (= (congestion-factor road_0321) 1.2)
  (= (travel-duration road_0321) 6.3276)
  (connects road_0323 loc_0188 loc_0262)
  (road-open road_0323)
  (= (congestion-factor road_0323) 1.0)
  (= (travel-duration road_0323) 2.1973)
  (connects road_0324 loc_0188 loc_0003)
  (road-open road_0324)
  (= (congestion-factor road_0324) 1.05)
  (= (travel-duration road_0324) 13.5594)
  (connects road_0325 loc_0189 loc_0012)
  (road-open road_0325)
  (= (congestion-factor road_0325) 1.14)
  (= (travel-duration road_0325) 31.6767)
  (road-next road_0325 macro_0001)
  (road-next road_0325 road_0016)
  (road-next road_0325 road_0017)
  (connects road_0327 loc_0191 loc_0195)
  (road-open road_0327)
  (= (congestion-factor road_0327) 1.0)
  (= (travel-duration road_0327) 10.6108)
  (road-next road_0327 road_0335)
  (road-next road_0327 road_0336)
  (road-next road_0327 road_0337)
  (connects road_0328 loc_0192 loc_0212)
  (road-open road_0328)
  (= (congestion-factor road_0328) 1.0)
  (= (travel-duration road_0328) 1.6189)
  (road-next road_0328 road_0371)
  (road-next road_0328 road_0372)
  (connects road_0329 loc_0192 loc_0193)
  (road-open road_0329)
  (= (congestion-factor road_0329) 1.1)
  (= (travel-duration road_0329) 7.7395)
  (road-next road_0329 road_0331)
  (road-next road_0329 road_0332)
  (connects road_0330 loc_0192 loc_0075)
  (road-open road_0330)
  (= (congestion-factor road_0330) 1.03)
  (= (travel-duration road_0330) 20.668)
  (road-next road_0330 road_0122)
  (connects road_0331 loc_0193 loc_0102)
  (road-open road_0331)
  (= (congestion-factor road_0331) 1.05)
  (= (travel-duration road_0331) 1.069)
  (road-next road_0331 macro_0019)
  (road-next road_0331 road_0168)
  (road-next road_0331 road_0169)
  (connects road_0332 loc_0193 loc_0244)
  (road-open road_0332)
  (= (congestion-factor road_0332) 1.25)
  (= (travel-duration road_0332) 7.5843)
  (road-next road_0332 macro_0034)
  (connects road_0333 loc_0194 loc_0232)
  (road-open road_0333)
  (= (congestion-factor road_0333) 1.2)
  (= (travel-duration road_0333) 6.6876)
  (road-next road_0333 road_0398)
  (connects road_0334 loc_0194 loc_0233)
  (road-open road_0334)
  (= (congestion-factor road_0334) 1.15)
  (= (travel-duration road_0334) 5.9356)
  (road-next road_0334 macro_0032)
  (road-next road_0334 road_0400)
  (connects road_0335 loc_0195 loc_0191)
  (road-open road_0335)
  (= (congestion-factor road_0335) 1.05)
  (= (travel-duration road_0335) 11.1414)
  (road-next road_0335 road_0327)
  (connects road_0336 loc_0195 loc_0176)
  (road-open road_0336)
  (= (congestion-factor road_0336) 1.0)
  (= (travel-duration road_0336) 12.9161)
  (road-next road_0336 macro_0027)
  (road-next road_0336 road_0307)
  (connects road_0337 loc_0195 loc_0103)
  (road-open road_0337)
  (= (congestion-factor road_0337) 1.1)
  (= (travel-duration road_0337) 1.6435)
  (road-next road_0337 road_0171)
  (road-next road_0337 road_0172)
  (connects road_0338 loc_0196 loc_0103)
  (road-open road_0338)
  (= (congestion-factor road_0338) 1.0)
  (= (travel-duration road_0338) 13.2017)
  (road-next road_0338 road_0171)
  (road-next road_0338 road_0172)
  (connects road_0339 loc_0196 loc_0100)
  (road-open road_0339)
  (= (congestion-factor road_0339) 1.1)
  (= (travel-duration road_0339) 12.1432)
  (road-next road_0339 road_0163)
  (road-next road_0339 road_0164)
  (connects road_0340 loc_0197 loc_0196)
  (road-open road_0340)
  (= (congestion-factor road_0340) 1.1)
  (= (travel-duration road_0340) 7.4953)
  (road-next road_0340 road_0338)
  (road-next road_0340 road_0339)
  (connects road_0341 loc_0197 loc_0198)
  (road-open road_0341)
  (= (congestion-factor road_0341) 1.1)
  (= (travel-duration road_0341) 9.6034)
  (road-next road_0341 road_0342)
  (connects road_0342 loc_0198 loc_0197)
  (road-open road_0342)
  (= (congestion-factor road_0342) 1.05)
  (= (travel-duration road_0342) 9.1669)
  (road-next road_0342 road_0340)
  (road-next road_0342 road_0341)
  (connects road_0344 loc_0199 loc_0102)
  (road-open road_0344)
  (= (congestion-factor road_0344) 1.0)
  (= (travel-duration road_0344) 13.2977)
  (road-next road_0344 macro_0019)
  (road-next road_0344 road_0168)
  (road-next road_0344 road_0169)
  (connects road_0345 loc_0200 loc_0230)
  (road-open road_0345)
  (= (congestion-factor road_0345) 1.1)
  (= (travel-duration road_0345) 2.4078)
  (road-next road_0345 road_0395)
  (connects road_0346 loc_0200 loc_0041)
  (road-open road_0346)
  (= (congestion-factor road_0346) 1.09)
  (= (travel-duration road_0346) 9.6018)
  (road-next road_0346 road_0065)
  (connects road_0347 loc_0201 loc_0203)
  (road-open road_0347)
  (= (congestion-factor road_0347) 1.35)
  (= (travel-duration road_0347) 4.0534)
  (road-next road_0347 road_0351)
  (road-next road_0347 road_0352)
  (road-next road_0347 road_0353)
  (connects road_0348 loc_0201 loc_0101)
  (road-open road_0348)
  (= (congestion-factor road_0348) 1.1)
  (= (travel-duration road_0348) 0.9808)
  (road-next road_0348 road_0165)
  (road-next road_0348 road_0166)
  (road-next road_0348 road_0167)
  (connects road_0349 loc_0202 loc_0205)
  (road-open road_0349)
  (= (congestion-factor road_0349) 1.25)
  (= (travel-duration road_0349) 3.1051)
  (road-next road_0349 road_0357)
  (road-next road_0349 road_0358)
  (road-next road_0349 road_0359)
  (connects road_0350 loc_0202 loc_0203)
  (road-open road_0350)
  (= (congestion-factor road_0350) 1.1)
  (= (travel-duration road_0350) 1.1418)
  (road-next road_0350 road_0351)
  (road-next road_0350 road_0352)
  (road-next road_0350 road_0353)
  (connects road_0351 loc_0203 loc_0210)
  (road-open road_0351)
  (= (congestion-factor road_0351) 1.3)
  (= (travel-duration road_0351) 46.3421)
  (road-next road_0351 road_0366)
  (road-next road_0351 road_0367)
  (connects road_0352 loc_0203 loc_0201)
  (road-open road_0352)
  (= (congestion-factor road_0352) 1.0)
  (= (travel-duration road_0352) 3.0025)
  (road-next road_0352 road_0347)
  (road-next road_0352 road_0348)
  (connects road_0353 loc_0203 loc_0202)
  (road-open road_0353)
  (= (congestion-factor road_0353) 1.1)
  (= (travel-duration road_0353) 1.1418)
  (road-next road_0353 road_0349)
  (road-next road_0353 road_0350)
  (connects road_0354 loc_0204 loc_0197)
  (road-open road_0354)
  (= (congestion-factor road_0354) 1.15)
  (= (travel-duration road_0354) 10.9977)
  (road-next road_0354 road_0340)
  (road-next road_0354 road_0341)
  (connects road_0355 loc_0204 loc_0205)
  (road-open road_0355)
  (= (congestion-factor road_0355) 1.25)
  (= (travel-duration road_0355) 2.3116)
  (road-next road_0355 road_0357)
  (road-next road_0355 road_0358)
  (road-next road_0355 road_0359)
  (connects road_0356 loc_0204 loc_0101)
  (road-open road_0356)
  (= (congestion-factor road_0356) 1.05)
  (= (travel-duration road_0356) 1.6809)
  (road-next road_0356 road_0165)
  (road-next road_0356 road_0166)
  (road-next road_0356 road_0167)
  (connects road_0357 loc_0205 loc_0074)
  (road-open road_0357)
  (= (congestion-factor road_0357) 1.25)
  (= (travel-duration road_0357) 27.6529)
  (road-next road_0357 road_0121)
  (connects road_0358 loc_0205 loc_0202)
  (road-open road_0358)
  (= (congestion-factor road_0358) 1.05)
  (= (travel-duration road_0358) 2.6083)
  (road-next road_0358 road_0349)
  (road-next road_0358 road_0350)
  (connects road_0359 loc_0205 loc_0204)
  (road-open road_0359)
  (= (congestion-factor road_0359) 1.1)
  (= (travel-duration road_0359) 2.0342)
  (road-next road_0359 road_0354)
  (road-next road_0359 road_0355)
  (road-next road_0359 road_0356)
  (connects road_0360 loc_0206 loc_0295)
  (road-open road_0360)
  (= (congestion-factor road_0360) 1.05)
  (= (travel-duration road_0360) 8.7977)
  (road-next road_0360 road_0496)
  (road-next road_0360 road_0497)
  (connects road_0361 loc_0207 loc_0034)
  (road-open road_0361)
  (= (congestion-factor road_0361) 1.0)
  (= (travel-duration road_0361) 3.6733)
  (road-next road_0361 macro_0005)
  (road-next road_0361 road_0053)
  (connects road_0364 loc_0209 loc_0208)
  (road-open road_0364)
  (= (congestion-factor road_0364) 1.0)
  (= (travel-duration road_0364) 1.6873)
  (connects road_0366 loc_0210 loc_0041)
  (road-open road_0366)
  (= (congestion-factor road_0366) 1.23)
  (= (travel-duration road_0366) 9.4617)
  (road-next road_0366 road_0065)
  (connects road_0367 loc_0210 loc_0230)
  (road-open road_0367)
  (= (congestion-factor road_0367) 1.11)
  (= (travel-duration road_0367) 3.0211)
  (road-next road_0367 road_0395)
  (connects road_0368 loc_0211 loc_0293)
  (road-open road_0368)
  (= (congestion-factor road_0368) 1.2)
  (= (travel-duration road_0368) 8.0053)
  (road-next road_0368 macro_0037)
  (road-next road_0368 macro_0038)
  (connects road_0369 loc_0211 loc_0248)
  (road-open road_0369)
  (= (congestion-factor road_0369) 1.15)
  (= (travel-duration road_0369) 12.6454)
  (road-next road_0369 road_0424)
  (connects road_0370 loc_0211 loc_0212)
  (road-open road_0370)
  (= (congestion-factor road_0370) 1.06)
  (= (travel-duration road_0370) 13.2357)
  (road-next road_0370 road_0371)
  (road-next road_0370 road_0372)
  (connects road_0371 loc_0212 loc_0192)
  (road-open road_0371)
  (= (congestion-factor road_0371) 1.06)
  (= (travel-duration road_0371) 1.716)
  (road-next road_0371 road_0328)
  (road-next road_0371 road_0329)
  (road-next road_0371 road_0330)
  (connects road_0372 loc_0212 loc_0141)
  (road-open road_0372)
  (= (congestion-factor road_0372) 1.05)
  (= (travel-duration road_0372) 12.0927)
  (road-next road_0372 road_0250)
  (road-next road_0372 road_0251)
  (connects road_0373 loc_0213 loc_0135)
  (road-open road_0373)
  (= (congestion-factor road_0373) 1.0)
  (= (travel-duration road_0373) 5.2346)
  (road-next road_0373 road_0235)
  (road-next road_0373 road_0236)
  (road-next road_0373 road_0237)
  (connects road_0374 loc_0214 loc_0134)
  (road-open road_0374)
  (= (congestion-factor road_0374) 1.0)
  (= (travel-duration road_0374) 3.0601)
  (road-next road_0374 road_0232)
  (road-next road_0374 road_0233)
  (road-next road_0374 road_0234)
  (connects road_0375 loc_0215 loc_0054)
  (road-open road_0375)
  (= (congestion-factor road_0375) 1.06)
  (= (travel-duration road_0375) 17.7451)
  (road-next road_0375 macro_0011)
  (road-next road_0375 macro_0012)
  (connects road_0376 loc_0216 loc_0123)
  (road-open road_0376)
  (= (congestion-factor road_0376) 1.14)
  (= (travel-duration road_0376) 21.6946)
  (road-next road_0376 road_0211)
  (road-next road_0376 road_0212)
  (connects road_0377 loc_0217 loc_0239)
  (road-open road_0377)
  (= (congestion-factor road_0377) 1.35)
  (= (travel-duration road_0377) 9.8678)
  (road-next road_0377 road_0409)
  (road-next road_0377 road_0410)
  (connects road_0379 loc_0219 loc_0253)
  (road-open road_0379)
  (= (congestion-factor road_0379) 1.25)
  (= (travel-duration road_0379) 4.1372)
  (road-next road_0379 road_0430)
  (road-next road_0379 road_0431)
  (connects road_0380 loc_0220 loc_0038)
  (road-open road_0380)
  (= (congestion-factor road_0380) 1.06)
  (= (travel-duration road_0380) 1.0571)
  (road-next road_0380 macro_0008)
  (road-next road_0380 road_0059)
  (road-next road_0380 road_0061)
  (connects road_0381 loc_0220 loc_0020)
  (road-open road_0381)
  (= (congestion-factor road_0381) 1.2)
  (= (travel-duration road_0381) 8.7541)
  (road-next road_0381 road_0028)
  (connects road_0382 loc_0221 loc_0220)
  (road-open road_0382)
  (= (congestion-factor road_0382) 1.11)
  (= (travel-duration road_0382) 7.3623)
  (road-next road_0382 road_0380)
  (road-next road_0382 road_0381)
  (connects road_0383 loc_0221 loc_0020)
  (road-open road_0383)
  (= (congestion-factor road_0383) 1.14)
  (= (travel-duration road_0383) 6.5694)
  (road-next road_0383 road_0028)
  (connects road_0384 loc_0222 loc_0021)
  (road-open road_0384)
  (= (congestion-factor road_0384) 1.29)
  (= (travel-duration road_0384) 1.2818)
  (road-next road_0384 macro_0004)
  (road-next road_0384 road_0030)
  (connects road_0385 loc_0223 loc_0037)
  (road-open road_0385)
  (= (congestion-factor road_0385) 1.03)
  (= (travel-duration road_0385) 15.0452)
  (road-next road_0385 road_0058)
  (connects road_0386 loc_0223 loc_0019)
  (road-open road_0386)
  (= (congestion-factor road_0386) 1.23)
  (= (travel-duration road_0386) 7.3537)
  (road-next road_0386 road_0027)
  (connects road_0387 loc_0224 loc_0223)
  (road-open road_0387)
  (= (congestion-factor road_0387) 1.06)
  (= (travel-duration road_0387) 6.3221)
  (road-next road_0387 road_0385)
  (road-next road_0387 road_0386)
  (connects road_0388 loc_0224 loc_0019)
  (road-open road_0388)
  (= (congestion-factor road_0388) 1.09)
  (= (travel-duration road_0388) 6.4605)
  (road-next road_0388 road_0027)
  (connects road_0390 loc_0226 loc_0227)
  (road-open road_0390)
  (= (congestion-factor road_0390) 1.06)
  (= (travel-duration road_0390) 1.9446)
  (road-next road_0390 road_0391)
  (connects road_0391 loc_0227 loc_0228)
  (road-open road_0391)
  (= (congestion-factor road_0391) 1.18)
  (= (travel-duration road_0391) 1.0968)
  (road-next road_0391 road_0392)
  (connects road_0392 loc_0228 loc_0229)
  (road-open road_0392)
  (= (congestion-factor road_0392) 1.18)
  (= (travel-duration road_0392) 2.344)
  (road-next road_0392 macro_0031)
  (road-next road_0392 road_0393)
  (connects road_0393 loc_0229 loc_0075)
  (road-open road_0393)
  (= (congestion-factor road_0393) 1.35)
  (= (travel-duration road_0393) 23.3549)
  (road-next road_0393 road_0122)
  (connects road_0395 loc_0230 loc_0179)
  (road-open road_0395)
  (= (congestion-factor road_0395) 1.17)
  (= (travel-duration road_0395) 21.0075)
  (road-next road_0395 road_0313)
  (road-next road_0395 road_0314)
  (connects road_0396 loc_0231 loc_0293)
  (road-open road_0396)
  (= (congestion-factor road_0396) 1.05)
  (= (travel-duration road_0396) 10.2203)
  (road-next road_0396 macro_0037)
  (road-next road_0396 macro_0038)
  (connects road_0397 loc_0231 loc_0201)
  (road-open road_0397)
  (= (congestion-factor road_0397) 1.0)
  (= (travel-duration road_0397) 22.5321)
  (road-next road_0397 road_0347)
  (road-next road_0397 road_0348)
  (connects road_0398 loc_0232 loc_0178)
  (road-open road_0398)
  (= (congestion-factor road_0398) 1.2)
  (= (travel-duration road_0398) 7.5833)
  (road-next road_0398 road_0311)
  (road-next road_0398 road_0312)
  (connects road_0400 loc_0233 loc_0077)
  (road-open road_0400)
  (= (congestion-factor road_0400) 1.2)
  (= (travel-duration road_0400) 13.9095)
  (road-next road_0400 road_0124)
  (connects road_0401 loc_0234 loc_0113)
  (road-open road_0401)
  (= (congestion-factor road_0401) 1.15)
  (= (travel-duration road_0401) 2.5296)
  (road-next road_0401 road_0192)
  (road-next road_0401 road_0193)
  (road-next road_0401 road_0194)
  (connects road_0402 loc_0235 loc_0236)
  (road-open road_0402)
  (= (congestion-factor road_0402) 1.0)
  (= (travel-duration road_0402) 19.028)
  (road-next road_0402 road_0403)
  (road-next road_0402 road_0404)
  (connects road_0403 loc_0236 loc_0235)
  (road-open road_0403)
  (= (congestion-factor road_0403) 1.1)
  (= (travel-duration road_0403) 20.9308)
  (road-next road_0403 road_0402)
  (connects road_0404 loc_0236 loc_0297)
  (road-open road_0404)
  (= (congestion-factor road_0404) 1.02)
  (= (travel-duration road_0404) 3.0228)
  (road-next road_0404 road_0499)
  (road-next road_0404 road_0500)
  (connects road_0405 loc_0237 loc_0275)
  (road-open road_0405)
  (= (congestion-factor road_0405) 1.05)
  (= (travel-duration road_0405) 96.182)
  (road-next road_0405 road_0465)
  (connects road_0406 loc_0237 loc_0276)
  (road-open road_0406)
  (= (congestion-factor road_0406) 1.0)
  (= (travel-duration road_0406) 0.9403)
  (road-next road_0406 road_0466)
  (road-next road_0406 road_0467)
  (connects road_0407 loc_0238 loc_0108)
  (road-open road_0407)
  (= (congestion-factor road_0407) 1.05)
  (= (travel-duration road_0407) 2.8628)
  (road-next road_0407 road_0184)
  (road-next road_0407 road_0185)
  (connects road_0408 loc_0238 loc_0173)
  (road-open road_0408)
  (= (congestion-factor road_0408) 1.15)
  (= (travel-duration road_0408) 9.6756)
  (road-next road_0408 road_0302)
  (road-next road_0408 road_0303)
  (connects road_0409 loc_0239 loc_0111)
  (road-open road_0409)
  (= (congestion-factor road_0409) 1.05)
  (= (travel-duration road_0409) 3.285)
  (road-next road_0409 road_0189)
  (road-next road_0409 road_0190)
  (connects road_0410 loc_0239 loc_0238)
  (road-open road_0410)
  (= (congestion-factor road_0410) 1.3)
  (= (travel-duration road_0410) 6.9064)
  (road-next road_0410 road_0407)
  (road-next road_0410 road_0408)
  (connects road_0411 loc_0240 loc_0241)
  (road-open road_0411)
  (= (congestion-factor road_0411) 1.0)
  (= (travel-duration road_0411) 10.414)
  (road-next road_0411 macro_0033)
  (road-next road_0411 road_0414)
  (connects road_0412 loc_0240 loc_0249)
  (road-open road_0412)
  (= (congestion-factor road_0412) 1.4)
  (= (travel-duration road_0412) 45.342)
  (road-next road_0412 road_0425)
  (connects road_0414 loc_0241 loc_0240)
  (road-open road_0414)
  (= (congestion-factor road_0414) 1.35)
  (= (travel-duration road_0414) 14.0589)
  (road-next road_0414 road_0411)
  (road-next road_0414 road_0412)
  (connects road_0415 loc_0242 loc_0102)
  (road-open road_0415)
  (= (congestion-factor road_0415) 1.3)
  (= (travel-duration road_0415) 9.0187)
  (road-next road_0415 macro_0019)
  (road-next road_0415 road_0168)
  (road-next road_0415 road_0169)
  (connects road_0416 loc_0242 loc_0244)
  (road-open road_0416)
  (= (congestion-factor road_0416) 1.05)
  (= (travel-duration road_0416) 1.7502)
  (road-next road_0416 macro_0034)
  (connects road_0417 loc_0243 loc_0292)
  (road-open road_0417)
  (= (congestion-factor road_0417) 1.4)
  (= (travel-duration road_0417) 1.5221)
  (road-next road_0417 road_0489)
  (road-next road_0417 road_0490)
  (connects road_0419 loc_0245 loc_0195)
  (road-open road_0419)
  (= (congestion-factor road_0419) 1.1)
  (= (travel-duration road_0419) 11.1795)
  (road-next road_0419 road_0335)
  (road-next road_0419 road_0336)
  (road-next road_0419 road_0337)
  (connects road_0420 loc_0245 loc_0243)
  (road-open road_0420)
  (= (congestion-factor road_0420) 1.2)
  (= (travel-duration road_0420) 2.1572)
  (road-next road_0420 road_0417)
  (connects road_0422 loc_0247 loc_0200)
  (road-open road_0422)
  (= (congestion-factor road_0422) 1.11)
  (= (travel-duration road_0422) 16.2977)
  (road-next road_0422 road_0345)
  (road-next road_0422 road_0346)
  (connects road_0423 loc_0247 loc_0287)
  (road-open road_0423)
  (= (congestion-factor road_0423) 1.2)
  (= (travel-duration road_0423) 2.3665)
  (road-next road_0423 road_0481)
  (connects road_0424 loc_0248 loc_0211)
  (road-open road_0424)
  (= (congestion-factor road_0424) 1.05)
  (= (travel-duration road_0424) 11.5458)
  (road-next road_0424 road_0368)
  (road-next road_0424 road_0369)
  (road-next road_0424 road_0370)
  (connects road_0425 loc_0249 loc_0187)
  (road-open road_0425)
  (= (congestion-factor road_0425) 1.31)
  (= (travel-duration road_0425) 2.7197)
  (road-next road_0425 macro_0029)
  (road-next road_0425 road_0321)
  (connects road_0426 loc_0250 loc_0054)
  (road-open road_0426)
  (= (congestion-factor road_0426) 1.0)
  (= (travel-duration road_0426) 4.3142)
  (road-next road_0426 macro_0011)
  (road-next road_0426 macro_0012)
  (connects road_0430 loc_0253 loc_0083)
  (road-open road_0430)
  (= (congestion-factor road_0430) 1.1)
  (= (travel-duration road_0430) 10.0944)
  (road-next road_0430 road_0134)
  (road-next road_0430 road_0135)
  (connects road_0431 loc_0253 loc_0091)
  (road-open road_0431)
  (= (congestion-factor road_0431) 1.15)
  (= (travel-duration road_0431) 18.8046)
  (road-next road_0431 road_0146)
  (road-next road_0431 road_0147)
  (connects road_0432 loc_0254 loc_0255)
  (road-open road_0432)
  (= (congestion-factor road_0432) 1.05)
  (= (travel-duration road_0432) 10.9801)
  (road-next road_0432 macro_0035)
  (road-next road_0432 road_0435)
  (connects road_0433 loc_0254 loc_0257)
  (road-open road_0433)
  (= (congestion-factor road_0433) 1.15)
  (= (travel-duration road_0433) 22.1637)
  (road-next road_0433 road_0437)
  (road-next road_0433 road_0438)
  (connects road_0435 loc_0255 loc_0057)
  (road-open road_0435)
  (= (congestion-factor road_0435) 1.0)
  (= (travel-duration road_0435) 19.0112)
  (road-next road_0435 macro_0013)
  (road-next road_0435 road_0097)
  (connects road_0437 loc_0257 loc_0259)
  (road-open road_0437)
  (= (congestion-factor road_0437) 1.25)
  (= (travel-duration road_0437) 1.4746)
  (road-next road_0437 road_0439)
  (road-next road_0437 road_0440)
  (connects road_0438 loc_0257 loc_0091)
  (road-open road_0438)
  (= (congestion-factor road_0438) 1.05)
  (= (travel-duration road_0438) 23.9876)
  (road-next road_0438 road_0146)
  (road-next road_0438 road_0147)
  (connects road_0439 loc_0259 loc_0257)
  (road-open road_0439)
  (= (congestion-factor road_0439) 1.0)
  (= (travel-duration road_0439) 1.1796)
  (road-next road_0439 road_0437)
  (road-next road_0439 road_0438)
  (connects road_0440 loc_0259 loc_0260)
  (road-open road_0440)
  (= (congestion-factor road_0440) 1.14)
  (= (travel-duration road_0440) 1.2162)
  (road-next road_0440 road_0441)
  (road-next road_0440 road_0442)
  (connects road_0441 loc_0260 loc_0261)
  (road-open road_0441)
  (= (congestion-factor road_0441) 1.09)
  (= (travel-duration road_0441) 3.3865)
  (road-next road_0441 road_0443)
  (road-next road_0441 road_0444)
  (connects road_0442 loc_0260 loc_0258)
  (road-open road_0442)
  (= (congestion-factor road_0442) 1.09)
  (= (travel-duration road_0442) 1.0844)
  (connects road_0443 loc_0261 loc_0136)
  (road-open road_0443)
  (= (congestion-factor road_0443) 1.06)
  (= (travel-duration road_0443) 21.8627)
  (road-next road_0443 road_0238)
  (road-next road_0443 road_0239)
  (road-next road_0443 road_0240)
  (road-next road_0443 road_0241)
  (connects road_0444 loc_0261 loc_0258)
  (road-open road_0444)
  (= (congestion-factor road_0444) 1.06)
  (= (travel-duration road_0444) 3.2933)
  (connects road_0445 loc_0263 loc_0158)
  (road-open road_0445)
  (= (congestion-factor road_0445) 1.09)
  (= (travel-duration road_0445) 5.3813)
  (road-next road_0445 road_0278)
  (road-next road_0445 road_0279)
  (connects road_0446 loc_0263 loc_0159)
  (road-open road_0446)
  (= (congestion-factor road_0446) 1.06)
  (= (travel-duration road_0446) 3.3786)
  (road-next road_0446 road_0280)
  (connects road_0447 loc_0264 loc_0078)
  (road-open road_0447)
  (= (congestion-factor road_0447) 1.05)
  (= (travel-duration road_0447) 7.5565)
  (road-next road_0447 road_0125)
  (connects road_0448 loc_0264 loc_0266)
  (road-open road_0448)
  (= (congestion-factor road_0448) 1.11)
  (= (travel-duration road_0448) 2.6934)
  (road-next road_0448 road_0451)
  (road-next road_0448 road_0452)
  (connects road_0449 loc_0265 loc_0194)
  (road-open road_0449)
  (= (congestion-factor road_0449) 1.35)
  (= (travel-duration road_0449) 10.2356)
  (road-next road_0449 road_0333)
  (road-next road_0449 road_0334)
  (connects road_0450 loc_0265 loc_0232)
  (road-open road_0450)
  (= (congestion-factor road_0450) 1.06)
  (= (travel-duration road_0450) 4.7295)
  (road-next road_0450 road_0398)
  (connects road_0451 loc_0266 loc_0290)
  (road-open road_0451)
  (= (congestion-factor road_0451) 1.0)
  (= (travel-duration road_0451) 10.9336)
  (road-next road_0451 road_0485)
  (road-next road_0451 road_0486)
  (connects road_0452 loc_0266 loc_0042)
  (road-open road_0452)
  (= (congestion-factor road_0452) 1.11)
  (= (travel-duration road_0452) 7.7952)
  (road-next road_0452 road_0066)
  (road-next road_0452 road_0067)
  (road-next road_0452 road_0068)
  (road-next road_0452 road_0069)
  (connects road_0453 loc_0267 loc_0052)
  (road-open road_0453)
  ;; [DYNAMIC EVENT - location closure] loc_0267 incident road
  (road-blocked road_0453)
  (= (congestion-factor road_0453) 1.0)
  (= (travel-duration road_0453) 15.8478)
  (road-next road_0453 macro_0009)
  (road-next road_0453 road_0088)
  (connects road_0454 loc_0267 loc_0094)
  (road-open road_0454)
  ;; [DYNAMIC EVENT - roadworks] Roadworks on roads 0152, 0153 and 0454 at intersection loc_0094 block a key junction near the corridor, forcing traffic to detour around the shared node.
  (road-blocked road_0454)
  (= (congestion-factor road_0454) 1.0)
  (= (travel-duration road_0454) 7.2351)
  (road-next road_0454 road_0151)
  (road-next road_0454 road_0152)
  (road-next road_0454 road_0153)
  (connects road_0455 loc_0268 loc_0116)
  (road-open road_0455)
  (= (congestion-factor road_0455) 1.0)
  (= (travel-duration road_0455) 10.8592)
  (road-next road_0455 road_0198)
  (road-next road_0455 road_0199)
  (road-next road_0455 road_0200)
  (connects road_0456 loc_0269 loc_0119)
  (road-open road_0456)
  (= (congestion-factor road_0456) 1.05)
  (= (travel-duration road_0456) 4.0725)
  (road-next road_0456 road_0204)
  (road-next road_0456 road_0205)
  (connects road_0457 loc_0270 loc_0143)
  (road-open road_0457)
  (= (congestion-factor road_0457) 1.05)
  (= (travel-duration road_0457) 19.2712)
  (road-next road_0457 road_0254)
  (connects road_0458 loc_0270 loc_0271)
  (road-open road_0458)
  (= (congestion-factor road_0458) 1.1)
  (= (travel-duration road_0458) 8.4167)
  (road-next road_0458 road_0459)
  (connects road_0459 loc_0271 loc_0270)
  (road-open road_0459)
  (= (congestion-factor road_0459) 1.0)
  (= (travel-duration road_0459) 7.6515)
  (road-next road_0459 road_0457)
  (road-next road_0459 road_0458)
  (connects road_0460 loc_0272 loc_0286)
  (road-open road_0460)
  (= (congestion-factor road_0460) 1.0)
  (= (travel-duration road_0460) 1.5721)
  (road-next road_0460 road_0480)
  (connects road_0461 loc_0272 loc_0219)
  (road-open road_0461)
  (= (congestion-factor road_0461) 1.05)
  (= (travel-duration road_0461) 0.8906)
  (road-next road_0461 road_0379)
  (connects road_0462 loc_0273 loc_0026)
  (road-open road_0462)
  (= (congestion-factor road_0462) 1.09)
  (= (travel-duration road_0462) 4.9798)
  (road-next road_0462 road_0038)
  (road-next road_0462 road_0039)
  (road-next road_0462 road_0040)
  (connects road_0463 loc_0273 loc_0027)
  (road-open road_0463)
  (= (congestion-factor road_0463) 1.25)
  (= (travel-duration road_0463) 2.7766)
  (road-next road_0463 road_0041)
  (road-next road_0463 road_0042)
  (connects road_0464 loc_0274 loc_0140)
  (road-open road_0464)
  (= (congestion-factor road_0464) 1.0)
  (= (travel-duration road_0464) 12.1313)
  (road-next road_0464 road_0247)
  (road-next road_0464 road_0248)
  (road-next road_0464 road_0249)
  (connects road_0465 loc_0275 loc_0237)
  (road-open road_0465)
  (= (congestion-factor road_0465) 1.0)
  (= (travel-duration road_0465) 91.6019)
  (road-next road_0465 road_0405)
  (road-next road_0465 road_0406)
  (connects road_0466 loc_0276 loc_0132)
  (road-open road_0466)
  (= (congestion-factor road_0466) 1.0)
  (= (travel-duration road_0466) 6.4727)
  (road-next road_0466 road_0227)
  (road-next road_0466 road_0228)
  (connects road_0467 loc_0276 loc_0277)
  (road-open road_0467)
  (= (congestion-factor road_0467) 1.02)
  (= (travel-duration road_0467) 1.211)
  (road-next road_0467 road_0468)
  (connects road_0468 loc_0277 loc_0298)
  (road-open road_0468)
  (= (congestion-factor road_0468) 1.06)
  (= (travel-duration road_0468) 0.9967)
  (road-next road_0468 road_0501)
  (road-next road_0468 road_0502)
  (connects road_0470 loc_0279 loc_0038)
  (road-open road_0470)
  (= (congestion-factor road_0470) 1.0)
  (= (travel-duration road_0470) 10.7932)
  (road-next road_0470 macro_0008)
  (road-next road_0470 road_0059)
  (road-next road_0470 road_0061)
  (connects road_0472 loc_0280 loc_0265)
  (road-open road_0472)
  (= (congestion-factor road_0472) 1.26)
  (= (travel-duration road_0472) 9.777)
  (road-next road_0472 road_0449)
  (road-next road_0472 road_0450)
  (connects road_0473 loc_0282 loc_0093)
  (road-open road_0473)
  ;; [DYNAMIC EVENT - location closure] loc_0093 incident road
  (road-blocked road_0473)
  (= (congestion-factor road_0473) 1.0)
  (= (travel-duration road_0473) 3.9278)
  (road-next road_0473 road_0149)
  (road-next road_0473 road_0150)
  (connects road_0474 loc_0283 loc_0112)
  (road-open road_0474)
  (= (congestion-factor road_0474) 1.05)
  (= (travel-duration road_0474) 0.344)
  (road-next road_0474 road_0191)
  (connects road_0475 loc_0283 loc_0172)
  (road-open road_0475)
  (= (congestion-factor road_0475) 1.15)
  (= (travel-duration road_0475) 27.619)
  (road-next road_0475 road_0301)
  (connects road_0476 loc_0283 loc_0173)
  (road-open road_0476)
  (= (congestion-factor road_0476) 1.0)
  (= (travel-duration road_0476) 3.1105)
  (road-next road_0476 road_0302)
  (road-next road_0476 road_0303)
  (connects road_0478 loc_0285 loc_0144)
  (road-open road_0478)
  (= (congestion-factor road_0478) 1.0)
  (= (travel-duration road_0478) 29.4852)
  (road-next road_0478 road_0255)
  (connects road_0479 loc_0285 loc_0185)
  (road-open road_0479)
  (= (congestion-factor road_0479) 1.25)
  (= (travel-duration road_0479) 2.2561)
  (connects road_0480 loc_0286 loc_0273)
  (road-open road_0480)
  (= (congestion-factor road_0480) 1.09)
  (= (travel-duration road_0480) 2.0406)
  (road-next road_0480 road_0462)
  (road-next road_0480 road_0463)
  (connects road_0481 loc_0287 loc_0247)
  (road-open road_0481)
  (= (congestion-factor road_0481) 1.0)
  (= (travel-duration road_0481) 1.9721)
  (road-next road_0481 road_0422)
  (road-next road_0481 road_0423)
  (connects road_0482 loc_0288 loc_0128)
  (road-open road_0482)
  (= (congestion-factor road_0482) 1.15)
  (= (travel-duration road_0482) 3.6765)
  (road-next road_0482 road_0219)
  (connects road_0483 loc_0288 loc_0140)
  (road-open road_0483)
  (= (congestion-factor road_0483) 1.03)
  (= (travel-duration road_0483) 2.8825)
  (road-next road_0483 road_0247)
  (road-next road_0483 road_0248)
  (road-next road_0483 road_0249)
  (connects road_0484 loc_0289 loc_0280)
  (road-open road_0484)
  (= (congestion-factor road_0484) 1.11)
  (= (travel-duration road_0484) 13.6908)
  (road-next road_0484 macro_0036)
  (road-next road_0484 road_0472)
  (connects road_0485 loc_0290 loc_0117)
  (road-open road_0485)
  (= (congestion-factor road_0485) 1.03)
  (= (travel-duration road_0485) 0.3127)
  (road-next road_0485 road_0201)
  (road-next road_0485 road_0202)
  (connects road_0486 loc_0290 loc_0264)
  (road-open road_0486)
  (= (congestion-factor road_0486) 1.09)
  (= (travel-duration road_0486) 9.4952)
  (road-next road_0486 road_0447)
  (road-next road_0486 road_0448)
  (connects road_0487 loc_0291 loc_0118)
  (road-open road_0487)
  (= (congestion-factor road_0487) 1.1)
  (= (travel-duration road_0487) 17.9395)
  (road-next road_0487 road_0203)
  (connects road_0488 loc_0291 loc_0263)
  (road-open road_0488)
  (= (congestion-factor road_0488) 1.09)
  (= (travel-duration road_0488) 12.8686)
  (road-next road_0488 road_0445)
  (road-next road_0488 road_0446)
  (connects road_0489 loc_0292 loc_0100)
  (road-open road_0489)
  (= (congestion-factor road_0489) 1.2)
  (= (travel-duration road_0489) 15.3856)
  (road-next road_0489 road_0163)
  (road-next road_0489 road_0164)
  (connects road_0490 loc_0292 loc_0242)
  (road-open road_0490)
  (= (congestion-factor road_0490) 1.35)
  (= (travel-duration road_0490) 1.1551)
  (road-next road_0490 road_0415)
  (road-next road_0490 road_0416)
  (connects road_0493 loc_0294 loc_0175)
  (road-open road_0493)
  (= (congestion-factor road_0493) 1.1)
  (= (travel-duration road_0493) 5.337)
  (road-next road_0493 road_0306)
  (connects road_0494 loc_0294 loc_0299)
  (road-open road_0494)
  (= (congestion-factor road_0494) 1.0)
  (= (travel-duration road_0494) 2.6905)
  (road-next road_0494 road_0503)
  (connects road_0495 loc_0294 loc_0174)
  (road-open road_0495)
  (= (congestion-factor road_0495) 1.05)
  (= (travel-duration road_0495) 6.3897)
  (road-next road_0495 road_0304)
  (road-next road_0495 road_0305)
  (connects road_0496 loc_0295 loc_0251)
  (road-open road_0496)
  (= (congestion-factor road_0496) 1.15)
  (= (travel-duration road_0496) 1.1579)
  (connects road_0497 loc_0295 loc_0206)
  (road-open road_0497)
  (= (congestion-factor road_0497) 1.05)
  (= (travel-duration road_0497) 8.7977)
  (road-next road_0497 road_0360)
  (connects road_0498 loc_0296 loc_0045)
  (road-open road_0498)
  (= (congestion-factor road_0498) 1.2)
  (= (travel-duration road_0498) 31.6006)
  (road-next road_0498 road_0075)
  (road-next road_0498 road_0076)
  (road-next road_0498 road_0077)
  (connects road_0499 loc_0297 loc_0139)
  (road-open road_0499)
  (= (congestion-factor road_0499) 1.0)
  (= (travel-duration road_0499) 1.175)
  (road-next road_0499 road_0244)
  (road-next road_0499 road_0245)
  (road-next road_0499 road_0246)
  (connects road_0500 loc_0297 loc_0105)
  (road-open road_0500)
  (= (congestion-factor road_0500) 1.04)
  (= (travel-duration road_0500) 14.8164)
  (road-next road_0500 road_0175)
  (road-next road_0500 road_0176)
  (road-next road_0500 road_0177)
  (connects road_0501 loc_0298 loc_0017)
  (road-open road_0501)
  (= (congestion-factor road_0501) 1.04)
  (= (travel-duration road_0501) 6.1072)
  (road-next road_0501 road_0026)
  (connects road_0502 loc_0298 loc_0237)
  (road-open road_0502)
  (= (congestion-factor road_0502) 1.02)
  (= (travel-duration road_0502) 1.2132)
  (road-next road_0502 road_0405)
  (road-next road_0502 road_0406)
  (connects road_0503 loc_0299 loc_0294)
  (road-open road_0503)
  (= (congestion-factor road_0503) 1.05)
  (= (travel-duration road_0503) 2.825)
  (road-next road_0503 road_0493)
  (road-next road_0503 road_0494)
  (road-next road_0503 road_0495)
  (has-traffic-light loc_0012)
  (has-traffic-light loc_0041)
  (has-traffic-light loc_0051)
  (has-traffic-light loc_0074)
  (has-traffic-light loc_0109)
  (has-traffic-light loc_0180)
  (has-traffic-light loc_0210)
  (has-traffic-light loc_0249)
  ;; [DYNAMIC EVENT - accident] An accident on road_0149, the narrow link between loc_0093 and loc_0282, closes a low‑degree connector that is part of the main route, causing a detour.
  (location-blocked loc_0093)
  ;; [DYNAMIC EVENT - roadworks] Roadworks on roads 0152, 0153 and 0454 at intersection loc_0094 block a key junction near the corridor, forcing traffic to detour around the shared node.
  (location-blocked loc_0094)
  ;; [DYNAMIC EVENT - roadworks] Roadworks on roads 0152, 0153 and 0454 at intersection loc_0094 block a key junction near the corridor, forcing traffic to detour around the shared node.
  (location-blocked loc_0267)
  )

  (:goal (reached-goal car1))

  (:metric minimize (travel-time car1))
)
